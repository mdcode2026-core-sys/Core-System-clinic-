...
must(executionReconciliation, "has_tenant_permission(p_tenant_id,'patients:create')", "canonical create permission boundary missing");
must(executionReconciliation, "has_tenant_permission(p_tenant_id,'patients:update')", "canonical update permission boundary missing");
must(executionReconciliation, "v_best_path_b", "non-government-ID Path B missing");
must(executionReconciliation, "v_candidates = 1", "candidate uniqueness requirement missing");
must(executionReconciliation, "strong_conflict", "strong-conflict handling missing");
must(executionReconciliation, "attribute_provenance", "identity provenance metadata missing");
...
must(contract, '"contract_version": "1.2"', "reconciled Gate 03 execution contract version missing");
must(contract, "local authenticated E2E", "local E2E closure requirement missing");
