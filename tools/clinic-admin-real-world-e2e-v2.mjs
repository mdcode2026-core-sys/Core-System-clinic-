...
await step("appointment booking", async () => {
  ...
  const dateInputs = dialog.locator('input[type="date"]'); if (await dateInputs.count()) await dateInputs.first().fill(new Date().toISOString().slice(0, 10)); const timeInputs = dialog.locator('input[type="time"]'); if (await timeInputs.count() < 2) throw new Error("Appointment time inputs missing");
  const candidateSlots = [["09:00","09:30"],["09:30","10:00"],["10:00","10:30"],["10:30","11:00"],["11:00","11:30"],["11:30","12:00"],["12:00","12:30"],["13:00","13:30"],["13:30","14:00"],["14:00","14:30"],["14:30","15:00"],["15:00","15:30"],["15:30","16:00"],["16:00","16:30"],["16:30","17:00"]];
  ...
  for (const [start, end] of candidateSlots) {
    ...
    const errorCode = result?.error; lastConflict = String(errorCode ?? `HTTP_${status}`);
    if (!isRetryableBookingConflict(errorCode)) throw new Error(`Appointment booking failed at ${start}-${end}: ${lastConflict}; dialog: ${(await dialog.innerText()).slice(0, 1200)}`);
    console.log(`E2E_APPOINTMENT_SLOT_CONFLICT=${start}-${end} ERROR=${lastConflict}`); await page.waitForTimeout(150);
  }
