...
async function syncOnce() {
  ...
  for (const file of manifest.files || []) {
    ...
    state.files[file.id] = { status:"available", path:destination, mimeType:file.mime_type, updatedAt:new Date().toISOString() };
...
async function serveLocalFile(req,res,fileId) {
  const state = await readState(); const entry = state.files[fileId];
  if (!entry?.path) return json(res,404,{error:"Local file not found"});
  try { const stat = await fs.stat(entry.path); const data = await fs.readFile(entry.path); res.writeHead(200,{"content-type":entry.mimeType||"application/octet-stream","content-length":stat.size,"cache-control":"private, max-age=60","access-control-allow-origin":"*"}); res.end(data); }
  catch { return json(res,404,{error:"Local file not found"}); }
}
