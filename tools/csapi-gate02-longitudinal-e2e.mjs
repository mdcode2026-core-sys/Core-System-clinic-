...
// 20/21 — first stage → canonical Next Action → Agenda-owned booking.
  ...
  const followupTypeSelect=fuSelects.filter({has:page.locator('option[value="custom"]')}).first();
  const actionTypeSelect=fuSelects.filter({has:page.locator('option[value="review"]')}).first();
  await followupTypeSelect.waitFor({state:"visible",timeout:15000});
  await actionTypeSelect.waitFor({state:"visible",timeout:15000});
  await followupTypeSelect.selectOption("custom");
