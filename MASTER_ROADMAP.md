...
**Milestone 4 — Clinical & Business Modules:** Medical Workflow Completion, Accounting Enhancements, Reporting Expansion, Follow-up Enhancements. **Updated per ADR-005 (2026-07-31, frozen):** this milestone now has an explicit prerequisite — the Medical Master Libraries and Procedure/Service Catalog (Medical Specialty Library, Procedure Master Library with many-to-many specialty relationships, Clinic Procedure Catalog as a customization layer over the master library, and a Service Catalog distinct from Procedures). This is substantial new schema (`medical_specialties`, `procedure_master_library`, `procedure_specialty_map`, `services`, `service_procedures`, plus reworking `clinic_procedures` to reference the master library) — not started.

**Milestone 5 — Super Admin Platform:** Platform Administration, Tenant Management, Subscription Management, Global Analytics, Feature Flags, Monitoring, System Health. *(Underlying tables `subscription_plans`, `feature_flags`, `billing_events`, `tenant_devices` already exist — see `DATABASE_SCHEMA.md`.)* **Updated per ADR-003 (2026-07-31, frozen):** Subscription Management is now specified as a four-layer architecture (Base Plan / Add-ons / Resource Limits / License Engine), a configurable trial policy (default 14 days), and a formal lifecycle state machine (Trial → Active → Expiring Soon → Grace Period → Suspended → Reactivated → Cancelled). This milestone owns building and managing the License Engine and plan/add-on catalog; Milestone 2's Subscription Center consumes it.

**Cross-cutting — Branch-Ready Architecture (ADR-004, 2026-07-31, frozen):** the platform must be multi-branch-ready from day one via a minimal `branches` table + one default branch per tenant, with operational tables staying tenant-scoped until multi-branch operation is actually built in a future milestone. Recommended to ship early — alongside or just before Milestone 2, since Milestone 2's Subscription Center is specified to display Branch Usage.

**Cross-cutting — Everything Is a Module Principle (ADR-006, 2026-07-31, frozen):** every module (present and future — Radiology, Laboratory, Pharmacy, Prescription, CRM, Marketing, AI are now explicitly anticipated future modules) must support Feature Flag, License Control, Permission Control, Independent Configuration, and Independent Activation. Applied going forward once the License Engine (ADR-003) exists; not retroactively applied to the already-issued Milestone 3 package.
