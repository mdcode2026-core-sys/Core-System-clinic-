-- ADR-014 remediation: Follow-up automation delegates delivery queueing to Communications boundary.
...
begin
  ...
  for v_followup in select rf.id, rf.tenant_id, rf.patient_id, rf.channel, rf.message_body, rf.scheduled_for, rf.followup_type, rf.automation_rule_id from public.retention_followups rf where rf.execution_mode='automated' and rf.status='open' and rf.scheduled_for <= now() and rf.channel is not null and rf.delivery_status in ('pending','failed') loop
    if public.enqueue_followup_notification(v_followup.tenant_id, v_followup.id, v_followup.patient_id, v_followup.channel, v_followup.message_body, v_followup.scheduled_for, v_followup.followup_type, v_followup.automation_rule_id) then
      v_queued := v_queued + 1;
    end if;
  end loop;
