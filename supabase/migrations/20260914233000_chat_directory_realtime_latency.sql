-- Chat directory + low-latency staff messaging.
-- Communications remains the authoritative storage/permission boundary.
  ...
  staff as (
    ...
    where cu.tenant_id = me.tenant_id
      ...
      and (
        ...
        or cu.full_name ilike '%' || trim(p_query) || '%'
        or cu.email ilike '%' || trim(p_query) || '%'
      )
  )
  select
    staff.id,
...
revoke execute on function public.get_internal_chat_directory(text) from public;
revoke execute on function public.get_internal_chat_directory(text) from anon;
grant execute on function public.get_internal_chat_directory(text) to authenticated;

-- Chat delivery uses the existing RLS policies on communication_messages.
-- The table was not previously in supabase_realtime, which forced UI polling.
