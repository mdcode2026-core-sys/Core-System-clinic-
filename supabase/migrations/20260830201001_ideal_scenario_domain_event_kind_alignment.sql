...
begin
 ...
 select id into v_id from operational_work_items where tenant_id=p_tenant_id and source_type=p_source_type and source_id=p_source_id and status not in('completed','cancelled') limit 1;
 ...
 values(p_tenant_id,'request',p_title,p_event_type||' · required permission: '||p_required_permission,p_requested_by,p_patient_id,p_source_type,p_source_id,'normal','open') returning id into v_id;
