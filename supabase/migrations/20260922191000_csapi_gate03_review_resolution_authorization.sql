...
-- Gate 03: human review resolution is also a privileged Patient create path.
...
begin
  ...
  if p_decision not in ('LINK_EXISTING','CREATE_NEW')
     or v_first is null or v_father is null or v_family is null
  ...
  if p_decision='LINK_EXISTING' then
    ...
    from public.patient_identities
    where id=p_candidate_identity_id and identity_state='active';

    if v_identity is null then
      return jsonb_build_object('success',false,'error','PATIENT_MATCH_NOT_FOUND');
    end if;
    ...
    insert into public.patient_identity_match_audit(
      tenant_id,patient_identity_id,outcome,policy_version,score,candidate_count,evidence,actor_user_id
    )
    values(
      p_tenant_id,v_identity,'EXACT_MATCH','patient-match-v2-human-reviewed',80,1,
      jsonb_build_object('decision','LINK_EXISTING','candidate_identity_id',v_identity),
      v_user
    );
    ...
    return jsonb_build_object(
      ...
      'patient_identity_id',v_identity,'human_reviewed',true
    );
  end if;
  ...
  values(
    ...
    jsonb_build_object(
      'first_name','human_reviewed_registration',
      'father_name','human_reviewed_registration',
      'family_name','human_reviewed_registration',
      'mother_name','human_reviewed_registration',
      'date_of_birth','human_reviewed_registration',
      'gender','human_reviewed_registration',
      'phone','human_reviewed_registration',
      'email','human_reviewed_registration'
    ),
    jsonb_build_object('source','human_reviewed_registration','verification','staff_reviewed')
  )
  ...
  values(
    p_tenant_id,v_first,v_family,v_father,v_family,v_mother,
    v_national,v_phone,v_email,v_dob,v_age,v_age_ref,v_gender,'active'
  )
  returning id into v_patient;
  ...
  values(
    p_tenant_id,v_identity,'NO_MATCH','patient-match-v2-human-reviewed',0,1,
    jsonb_build_object('decision','CREATE_NEW','overrode_candidate',p_candidate_identity_id),
    v_user
  );
