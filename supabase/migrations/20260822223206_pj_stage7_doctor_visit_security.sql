...
-- Effective permission resolver for database-side RLS. Roles remain templates;
-- individual overrides are authoritative for the clinic user.
...
-- Existing Stage 6 policies are retained but their role-based predicates are replaced
-- with effective-permission predicates. This preserves the policy identities used by the system.
alter policy rls_sessions_select on public.clinic_visit_sessions
using (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:read'));
