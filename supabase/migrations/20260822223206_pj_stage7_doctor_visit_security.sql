...
create index if not exists clinic_visit_procedures_visit_idx on public.clinic_visit_procedures(tenant_id, visit_id);
create index if not exists clinic_visit_sessions_clinical_idx on public.clinic_visit_sessions(tenant_id, session_status, doctor_id);

insert into public.permissions (permission_key, permission_name, description, resource, action)
values ('visits:read', 'Read Visits', 'View clinical visit records', 'visits', 'read'), ('visits:update', 'Update Visits', 'Record and update clinical visit data', 'visits', 'update')
on conflict (permission_key) do nothing;

-- Roles are editable templates. The resolver starts with the user's role template,
-- falls back to role_key only for legacy users that have no template yet, then applies
-- the user's explicit override for that permission when one exists.
...
-- Existing Stage 6 policies are retained; their role-based predicates are replaced by
-- effective-permission predicates while keeping sessions:* permissions valid for Queue.
...
alter policy rls_sessions_write on public.clinic_visit_sessions
...
for select to authenticated using (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:read'));

create policy clinic_visit_procedures_insert on public.clinic_visit_procedures
for insert to authenticated with check (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:update'));

create policy clinic_visit_procedures_update on public.clinic_visit_procedures
for update to authenticated
using (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:update'))
with check (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:update'));

create policy clinic_visit_procedures_delete on public.clinic_visit_procedures
for delete to authenticated using (tenant_id = public.get_current_tenant_id() and public.has_effective_permission('visits:update'));
