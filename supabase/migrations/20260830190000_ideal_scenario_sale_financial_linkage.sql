-- Ideal Scenarios 25-28: a commercial sale must carry an existing, patient-owned financial plan and create its invoice from the same commercial amount.
...
declare v_price integer; v_discount integer:=0; v_net integer; v_patient_package uuid; v_invoice jsonb; v_financial_total integer; v_financial_patient integer;
 v_name text; v_invoice_id uuid;
begin
 ...
 v_invoice:=public.create_manual_invoice(p_tenant_id,p_patient_id,null,current_date,'financial_plan_sale','Commercial sale: '||v_name,p_created_by,jsonb_build_array(jsonb_build_object('procedure_id',null,'description',v_name,'quantity',1,'unit_price_subunits',v_price,'discount_amount_subunits',v_discount,'discount_percent',null,'tax_rate_percent',null)));
 ...
 if p_package_id is not null then
  ...
  select p_tenant_id,p_patient_id,id,p_financial_plan_id,coalesce(session_limit,0),0,'active',now(),p_created_by from clinic_packages where id=p_package_id and tenant_id=p_tenant_id returning id into v_patient_package;
 end if;
