-- Chat/Communications production QA repair.
-- Keep the existing Communications authorization model intact.
-- A sender authorized by communications:send must be able to see the conversation
-- row they just created while the same request adds its participants.
-- This breaks the RLS visibility deadlock without granting communications:manage
-- and without introducing a second messaging engine or authority model.
