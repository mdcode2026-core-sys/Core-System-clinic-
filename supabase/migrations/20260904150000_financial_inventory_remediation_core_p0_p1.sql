...
-- Preserves legacy experimental data; no destructive data operation.

begin;

alter table public.inventory_items add column if not exists purchase_cost_subunits integer, add column if not exists valuation_cost_subunits integer, add column if not exists selling_price_subunits integer, add column if not exists is_procedure_material boolean not null default false, add column if not exists is_operating_consumable boolean not null default false, add column if not exists requires_batch_tracking boolean not null default false, add column if not exists requires_expiry_tracking boolean not null default false;
alter table public.inventory_ledger add column if not exists quantity_delta numeric not null default 0, add column if not exists movement_type text not null default 'legacy_unclassified', add column if not exists source_type text, add column if not exists source_id uuid, add column if not exists unit_cost_subunits integer, add column if not exists cost_total_subunits integer;
update public.inventory_ledger set quantity_delta=quantity_consumed where quantity_delta=0 and quantity_consumed<>0;
update public.inventory_ledger set movement_type=case when consumption_type='purchase' then 'purchase_receipt' when consumption_type='purchase_return' then 'purchase_return' when consumption_type='unused_return' then 'unused_return' when consumption_type='doctor_request' then 'procedure_consumption' when consumption_type in ('inventory_adjustment_increase','inventory_adjustment_decrease') then 'adjustment' else 'legacy_unclassified' end where movement_type='legacy_unclassified';
update public.inventory_ledger set source_type='legacy_untraceable', notes=concat_ws(' | ',nullif(notes,''),'Legacy experimental movement preserved during 2026-09-04 remediation; source was not traceable at audit time.') where source_type is null and procedure_id is null and session_id is null and treatment_plan_item_id is null and quantity_delta<>0;

alter table public.inventory_items add constraint inventory_items_purchase_cost_nonnegative check (purchase_cost_subunits is null or purchase_cost_subunits>=0) not valid;
alter table public.inventory_items add constraint inventory_items_valuation_cost_nonnegative check (valuation_cost_subunits is null or valuation_cost_subunits>=0) not valid;
alter table public.inventory_items add constraint inventory_items_selling_price_nonnegative check (selling_price_subunits is null or selling_price_subunits>=0) not valid;
alter table public.inventory_ledger add constraint inventory_ledger_movement_type_check check (movement_type in ('purchase_receipt','purchase_return','procedure_consumption','unused_return','adjustment','legacy_unclassified')) not valid;
alter table public.inventory_ledger add constraint inventory_ledger_source_type_check check (source_type is null or source_type in ('purchase_receipt','purchase_return','procedure_session','treatment_plan_item','unused_return','manual_adjustment','legacy_untraceable')) not valid;
alter table public.clinic_invoices add constraint clinic_invoices_money_nonnegative check (subtotal_subunits>=0 and discount_subunits>=0 and tax_subunits>=0 and total_subunits>=0 and amount_paid_subunits>=0 and coalesce(amount_due_subunits,greatest(total_subunits-amount_paid_subunits,0))>=0) not valid;
alter table public.invoice_items add constraint invoice_items_money_nonnegative check (quantity>0 and unit_price_subunits>=0 and discount_subunits>=0 and tax_subunits>=0 and coalesce(line_total_subunits,0)>=0) not valid;
alter table public.invoice_payments add constraint invoice_payments_money_positive check (amount_subunits>0) not valid;

create or replace function public.guard_invoice_mutation() returns trigger language plpgsql set search_path to 'public' as $function$
begin
 if old.invoice_status in ('issued','paid','partial','refunded') then
  if new.tenant_id is distinct from old.tenant_id or new.patient_id is distinct from old.patient_id or new.session_id is distinct from old.session_id or new.invoice_date is distinct from old.invoice_date or new.invoice_number is distinct from old.invoice_number or new.subtotal_subunits is distinct from old.subtotal_subunits or new.discount_subunits is distinct from old.discount_subunits or new.discount_reason is distinct from old.discount_reason or new.discount_approved_by is distinct from old.discount_approved_by or new.tax_subunits is distinct from old.tax_subunits or new.total_subunits is distinct from old.total_subunits or new.payment_terms is distinct from old.payment_terms or new.notes is distinct from old.notes or new.deleted_at is distinct from old.deleted_at then raise exception 'Issued/paid invoice financial facts are immutable; use an approved lifecycle operation'; end if;
 end if;
 if old.invoice_status in ('paid','partial','refunded') and new.invoice_status not in ('paid','partial','refunded') then raise exception 'Paid/partial/refunded invoice cannot move backward in lifecycle'; end if;
 return new;
end;
$function$;
drop trigger if exists trg_guard_invoice_mutation on public.clinic_invoices;
create trigger trg_guard_invoice_mutation before update on public.clinic_invoices for each row execute function public.guard_invoice_mutation();
...
begin
 ...
 if v_actor is not null and not exists(select 1 from public.clinic_users where id=v_actor and tenant_id=p_tenant_id and is_active and deleted_at is null) then raise exception 'Actor does not belong to tenant'; end if;
 ...
 select valuation_cost_subunits into v_unit_cost from public.inventory_items where id=p_item_id and tenant_id=p_tenant_id and deleted_at is null for update;
 ...
 insert into public.inventory_ledger(tenant_id,item_id,procedure_id,material_name,quantity_consumed,consumption_type,logged_by,session_id,notes,treatment_plan_item_id,quantity_delta,movement_type,source_type,source_id,unit_cost_subunits,cost_total_subunits)
 select p_tenant_id,p_item_id,p_procedure_id,i.name,abs(p_delta)::numeric,v_consumption_type,v_actor,p_session_id,p_reason,p_treatment_plan_item_id,p_delta,p_movement_type,p_source_type,p_source_id,v_unit_cost,case when v_unit_cost is null then null else abs(p_delta)*v_unit_cost end from public.inventory_items i where i.id=p_item_id and i.tenant_id=p_tenant_id;
 return v_new_stock;
end;
$function$;

create or replace function public.issue_invoice(p_invoice_id uuid) returns jsonb language plpgsql set search_path to 'public' as $function$
declare v_invoice record; v_number text; v_tenant uuid;
begin
 select * into v_invoice from public.clinic_invoices where id=p_invoice_id; if not found then return jsonb_build_object('success',false,'error','Invoice not found'); end if;
 v_tenant:=public.get_current_tenant_id(); if v_tenant is distinct from v_invoice.tenant_id then return jsonb_build_object('success',false,'error','Tenant mismatch'); end if;
 ...
 v_number:=v_invoice.invoice_number; if v_number is null or btrim(v_number)='' then v_number:=public.generate_invoice_number(v_invoice.tenant_id); end if;
 update public.clinic_invoices set invoice_status='issued',issued_at=now(),invoice_number=v_number,updated_at=now() where id=p_invoice_id and tenant_id=v_tenant;
 return jsonb_build_object('success',true,'invoice_id',p_invoice_id,'status','issued','invoice_number',v_number);
end;
$function$;

create or replace function public.cancel_invoice(p_invoice_id uuid) returns jsonb language plpgsql set search_path to 'public' as $function$
...
begin
 ...
 select count(*) into v_payment_count from public.invoice_payments where invoice_id=p_invoice_id and tenant_id=v_tenant; if v_payment_count>0 then return jsonb_build_object('success',false,'error','Cannot cancel invoice with payments; use refund/reversal'); end if;
 if v_invoice.invoice_status='issued' and (v_invoice.issued_at is null or now()-v_invoice.issued_at>interval '15 minutes') then return jsonb_build_object('success',false,'error','Cancellation window expired (15 minutes)'); end if;
...
begin
 ...
 insert into public.invoice_payments(tenant_id,invoice_id,amount_subunits,payment_method,payment_reference,notes,collected_by) values(p_tenant_id,p_invoice_id,p_amount_subunits,p_payment_method,p_payment_reference,p_notes,v_actor) returning id into v_payment_id;
...
create or replace function public.create_invoice_from_session(p_session_id uuid) returns jsonb language plpgsql set search_path to 'public' as $function$
declare v_tenant_id uuid; v_invoice_id uuid; v_invoice_number text; v_patient_id uuid; v_subtotal integer:=0; v_tax_total integer:=0; v_total integer:=0; v_procedure record; v_item_subtotal integer; v_item_tax integer; v_item_total integer;
begin
 select tenant_id,patient_id into v_tenant_id,v_patient_id from public.clinic_visit_sessions where id=p_session_id; if v_patient_id is null then return jsonb_build_object('success',false,'error','Session not found or has no patient'); end if;
 ...
 insert into public.clinic_invoices(tenant_id,session_id,patient_id,invoice_number,subtotal_subunits,tax_subunits,total_subunits,amount_paid_subunits,amount_due_subunits,discount_subunits,invoice_status,payment_terms) values(v_tenant_id,p_session_id,v_patient_id,v_invoice_number,0,0,0,0,0,0,'draft','cash') returning id into v_invoice_id;
 for v_procedure in select cp.id procedure_id,cp.procedure_name,cp.procedure_name_ar,cp.base_price_subunits,cp.tax_included,cp.tax_rate_percent from public.clinic_visit_sessions cvs join public.master_agenda_events mae on mae.id=cvs.agenda_event_id join public.clinic_procedures cp on cp.id=mae.procedure_id where cvs.id=p_session_id and cvs.tenant_id=v_tenant_id and cp.is_active=true loop
  ...
  insert into public.invoice_items(tenant_id,invoice_id,procedure_id,item_description,item_description_ar,quantity,unit_price_subunits,tax_subunits,tax_rate_percent,line_total_subunits,sort_order) values(v_tenant_id,v_invoice_id,v_procedure.procedure_id,v_procedure.procedure_name,v_procedure.procedure_name_ar,1,v_item_subtotal,v_item_tax,v_procedure.tax_rate_percent,v_item_total,coalesce((select max(sort_order)+1 from public.invoice_items where invoice_id=v_invoice_id),0));
...
create or replace function public.apply_payment_to_installment(p_tenant_id uuid,p_installment_id uuid,p_amount_subunits integer,p_payment_method text default 'cash',p_payment_reference text default null,p_collected_by uuid default null) returns jsonb language plpgsql set search_path to 'public' as $function$
declare v_installment record; v_invoice record; v_paid integer; v_status text; v_payment jsonb; v_actor uuid;
begin
 if public.get_current_tenant_id() is distinct from p_tenant_id then return jsonb_build_object('success',false,'error','Tenant mismatch'); end if;
 if not public.has_tenant_permission(p_tenant_id,'invoices:payment') then return jsonb_build_object('success',false,'error','Permission denied'); end if;
 if p_amount_subunits<=0 then return jsonb_build_object('success',false,'error','Payment amount must be positive'); end if;
 v_actor:=coalesce(p_collected_by,auth.uid()); if v_actor is not null and not exists(select 1 from public.clinic_users where id=v_actor and tenant_id=p_tenant_id and is_active and deleted_at is null) then return jsonb_build_object('success',false,'error','Collector does not belong to tenant'); end if;
 select * into v_installment from public.financial_installments where id=p_installment_id and tenant_id=p_tenant_id and deleted_at is null for update; if not found then return jsonb_build_object('success',false,'error','Installment not found'); end if;
 ...
 select * into v_invoice from public.clinic_invoices where id=v_installment.invoice_id and tenant_id=p_tenant_id for update; if not found then return jsonb_build_object('success',false,'error','Installment invoice not found'); end if;
 v_payment:=public.record_invoice_payment(p_tenant_id,v_invoice.id,p_amount_subunits,p_payment_method,p_payment_reference,'Installment payment',v_actor); if coalesce((v_payment->>'success')::boolean,false) is not true then return v_payment; end if;
 v_paid:=v_installment.amount_paid_subunits+p_amount_subunits; v_status:=case when v_paid>=v_installment.amount_subunits then 'paid' when v_paid>0 then 'partial' else v_installment.status end;
