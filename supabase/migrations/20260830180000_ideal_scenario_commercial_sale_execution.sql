...
as $$
declare v_price integer; v_discount integer:=0; v_net integer; v_patient_package uuid;
begin
  ...
  if p_service_id is not null then select base_price_subunits into v_price from clinic_services where id=p_service_id and tenant_id=p_tenant_id and is_active; else select base_price_subunits into v_price from clinic_packages where id=p_package_id and tenant_id=p_tenant_id and is_active; end if;
  ...
  if p_package_id is not null then
    ...
    select p_tenant_id,p_patient_id,id,p_financial_plan_id,session_limit,0,'active',now(),p_created_by from clinic_packages where id=p_package_id and tenant_id=p_tenant_id returning id into v_patient_package;
  end if;
  return jsonb_build_object('success',true,'gross_subunits',v_price,'discount_subunits',v_discount,'net_subunits',v_net,'patient_package_id',v_patient_package,'financial_plan_id',p_financial_plan_id,'service_id',p_service_id,'package_id',p_package_id,'offer_id',p_offer_id);
end; $$;
