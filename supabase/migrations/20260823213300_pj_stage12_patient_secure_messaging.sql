...
create policy patient_messages_read on public.patient_portal_messages for select to authenticated using (
  ...
  or exists (select 1 from public.patient_portal_identities ppi join public.patient_clinic_relationships pcr on pcr.patient_identity_id=ppi.patient_identity_id join public.patient_identities pi on pcr.patient_identity_id=pi.id and pcr.clinic_patient_id=patient_portal_messages.clinic_patient_id and pcr.tenant_id=patient_portal_messages.tenant_id and pcr.status='active' where ppi.auth_user_id=(select auth.uid()) and ppi.status='active')
);
