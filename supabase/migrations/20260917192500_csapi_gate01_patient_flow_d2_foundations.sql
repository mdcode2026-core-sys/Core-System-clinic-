...
CREATE TABLE IF NOT EXISTS public.clinical_work_sessions (
  ...
  performed_by_clinic_user_id uuid NOT NULL, clinical_provider_id uuid, room_id uuid, started_at timestamptz, ended_at timestamptz,
  outcome text CHECK (outcome IS NULL OR outcome IN ('finish','hold','transfer','cancelled')), hold_reason text, transfer_reason text, version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), CONSTRAINT clinical_work_sessions_tenant_id_key UNIQUE (tenant_id,id),
...
CREATE TABLE IF NOT EXISTS public.patient_flow_queue_entries (
  ...
  position integer NOT NULL CHECK (position>=0), entered_at timestamptz NOT NULL DEFAULT now(), exited_at timestamptz, entry_reason text, exit_reason text, routing_target text, created_by_clinic_user_id uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), CONSTRAINT patient_flow_queue_entries_tenant_id_key UNIQUE (tenant_id,id),
...
CREATE TABLE IF NOT EXISTS public.patient_flow_events (
  ...
  destination_context jsonb NOT NULL DEFAULT '{}'::jsonb, reason text, metadata jsonb NOT NULL DEFAULT '{}'::jsonb, correlation_id uuid, created_at timestamptz NOT NULL DEFAULT now(),
