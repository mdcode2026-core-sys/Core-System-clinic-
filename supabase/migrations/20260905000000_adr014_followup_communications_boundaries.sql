...
BEGIN
  ...
  IF p_channel IN ('email','sms','whatsapp') THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE te.tenant_id = p_tenant_id
        AND te.entitlement_key = v_entitlement_key
  ...
  INSERT INTO public.notification_queue
    (tenant_id, recipient_type, recipient_id, channel, message_body, priority,
     status, retry_count, max_retries, scheduled_at, metadata)
  SELECT rf.tenant_id, 'patient', rf.patient_id, rf.channel,
    coalesce(rf.message_body,''), 5, 'queued', 0, 3, rf.scheduled_for,
  ...
  WHERE rf.id = p_followup_id AND rf.tenant_id = p_tenant_id
    AND rf.patient_id = p_patient_id AND rf.channel = p_channel
    AND rf.execution_mode = 'automated' AND rf.status = 'open'
    AND rf.scheduled_for <= now() AND rf.delivery_status IN ('pending','failed')
    AND NOT EXISTS (
      SELECT 1 FROM public.notification_queue nq
      WHERE nq.tenant_id = rf.tenant_id
...
-- run_followup_automation() now delegates delivery-queue creation to the
-- Communications delivery boundary function above. The function retains the
-- existing Follow-up rule evaluation and idempotent source-key behavior.
-- Its deployed definition is maintained by the corresponding production
-- migration application; this migration file records the canonical contract.
