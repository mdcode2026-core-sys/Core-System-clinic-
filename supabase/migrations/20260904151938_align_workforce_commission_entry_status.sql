-- Align calculated commission entries with the canonical workforce entry lifecycle.
-- Calculated entries are created in draft state; valid lifecycle values are draft/approved/paid/void.
...
declare v_payment record; v_rule record; v_employee record; v_existing uuid; v_eligible integer; v_commission integer; v_actor uuid; v_entry_id uuid;
begin
 ...
 select * into v_employee from public.workforce_employees where id=p_employee_id and tenant_id=p_tenant_id; if not found then return jsonb_build_object('success',false,'error','Employee not found'); end if;
 ...
 select id into v_existing from public.workforce_commission_entries where tenant_id=p_tenant_id and source_payment_id=p_payment_id and employee_id=p_employee_id and commission_rule_id=p_rule_id and status<>'void' limit 1; if v_existing is not null then return jsonb_build_object('success',true,'entry_id',v_existing,'idempotent',true); end if;
 v_eligible:=v_payment.amount_subunits; v_commission:=case when lower(v_rule.basis) in ('collected_revenue','invoice_value') then round(v_eligible*(v_rule.rate/100.0)) when lower(v_rule.basis)='fixed_bonus' then coalesce(v_rule.fixed_amount_subunits,0) else 0 end;
 insert into public.workforce_commission_entries(tenant_id,employee_id,commission_rule_id,source_type,source_id,basis_amount_subunits,eligible_amount_subunits,status,calculated_at,created_by,source_payment_id,notes) values(p_tenant_id,p_employee_id,p_rule_id,'invoice_payment',p_payment_id,v_payment.amount_subunits,v_eligible,'draft',now(),v_actor,p_payment_id,'Calculated from canonical invoice payment; amount derived from commission rule') returning id into v_entry_id;
