-- Governed domain-event -> operational-work bridge for Scenario 41.
create or replace function public.create_operational_work_from_domain_event(
 p_tenant_id uuid,p_event_type text,p_source_type text,p_source_id uuid,p_patient_id uuid,p_title text,p_required_permission text,p_requested_by uuid
) returns jsonb language plpgsql security definer set search_path=public as $$
declare v_id uuid;
begin
 if not exists(select 1 from clinic_users where id=p_requested_by and tenant_id=p_tenant_id and is_active) then return jsonb_build_object('success',false,'error','Invalid requester'); end if;
