begin;
-- Gate 03 corrective migration: reconcile live patient-match-v2 national ID weight to binding contract (20).
...
begin
  ...
  loop
    v_score := 0;
    if v_national is not null and exists (select 1 from public.patient_identity_identifiers pii where pii.patient_identity_id=r.id and pii.identifier_type='national_id' and pii.value_hash=digest(v_national,'sha256')) then v_score := v_score + 20; end if;
