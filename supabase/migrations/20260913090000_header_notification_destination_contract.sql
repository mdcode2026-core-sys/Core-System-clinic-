-- Header UX/UI destination binding: expose only producer-supplied authoritative paths.
-- The Header never derives routes from notification text or identifiers.
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'destination_path', nullif(nq.metadata->>'destination_path', '')
    ) as item
    from public.notification_queue nq
    where nq.tenant_id = v_tenant_id
      and nq.channel = 'in_app'
