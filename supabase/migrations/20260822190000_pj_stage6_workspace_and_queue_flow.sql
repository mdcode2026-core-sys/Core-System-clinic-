...
WHERE r.role_key IN ('receptionist','clinic_admin','super_admin')
  AND p.permission_key = 'workspace:operation'
...
WHERE r.role_key IN ('doctor','clinic_admin','super_admin')
  AND p.permission_key = 'workspace:clinical'
ON CONFLICT DO NOTHING;
...
FROM public.roles r
CROSS JOIN public.permissions p
WHERE r.role_key IN ('clinic_admin','super_admin')
  AND p.permission_key = 'workspace:administration'
ON CONFLICT DO NOTHING;
