...
-- Production-applied companion to 20260904150000_financial_inventory_remediation_core_p0_p1.sql

begin;

-- Canonical manual invoice path: creator/discount authority is tenant-bound.
...
-- Purchasing receiving now validates actor/supplier and delegates stock+ledger atomically.
...
begin
 ...
 select count(*) filter(where quantity_received<quantity_ordered) into v_remaining from public.purchase_order_items where purchase_order_id=p_purchase_order_id; v_status:=case when v_remaining=0 then 'received' else 'partially_received' end;
