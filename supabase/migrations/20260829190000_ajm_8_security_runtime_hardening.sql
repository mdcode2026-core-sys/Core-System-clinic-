...
create policy communications_participants_access on public.communication_conversation_participants for all to authenticated using (
 ...
 tenant_id=public.get_current_tenant_id() and public.has_tenant_permission(tenant_id,'communications:send')
);
...
create policy work_history_create on public.operational_work_history for insert to authenticated with check (
 tenant_id=public.get_current_tenant_id() and (
  public.has_tenant_permission(tenant_id,'work:create') or public.has_tenant_permission(tenant_id,'work:manage')
 ) and actor_clinic_user_id=(select id from public.clinic_users where auth_user_id=auth.uid() and tenant_id=operational_work_history.tenant_id and is_active=true and deleted_at is null limit 1)
);
...
create policy work_items_manage on public.operational_work_items for update to authenticated using (
 tenant_id=public.get_current_tenant_id() and (
  ...
  assignee_clinic_user_id=(select id from public.clinic_users where auth_user_id=auth.uid() and tenant_id=operational_work_items.tenant_id limit 1)
 )
