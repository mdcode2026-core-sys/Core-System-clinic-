...
- **BLOCKED** = a required Production path is not yet deployed or requires authenticated/email interaction unavailable to this execution context.
...
| 3 | Employee follows invitation to production `/activate` | Production `/activate` returns HTTP 200 and renders the activation experience. | PASS — route only |
...
| 10 | Pending user can Resend Invitation | Canonical lifecycle manager exposes Resend Invitation for `pending`; current production alias has not yet reached the latest lifecycle deployment. | BLOCKED — deployment pending |
...
| 16 | Employee can set new password through recovery | Recovery completion code exists at `/reset-password`, but current Production alias checked before its deployment became Ready returned 404. | BLOCKED — latest deployment pending |
...
| 24 | Activation/Recovery Show/Hide password works | Production `/activate` visibly contains both password fields and Show/Hide controls. Recovery route awaits latest deployment. | PASS — Activation; recovery pending |
...
## Production deployment state at matrix creation

The latest `main` deployment containing the final documentation commit was still **BUILDING**. Earlier production deployments already expose `/login`, `/forgot-password`, and `/activate`; `/reset-password` was not yet present on the production alias at the time of the check. Therefore the matrix intentionally does not claim full Production closure.

## Closure decision

**NOT CLOSED.**

The implementation and documentation are now aligned around the canonical lifecycle, but full closure requires:

1. Latest `main` deployment reaches `READY`.
2. `/reset-password` is verified on Production.
3. Authenticated end-to-end execution of the remaining lifecycle scenarios is performed with a controlled test employee.
4. Email receipt/verification and recovery are observed.
5. Deactivate/Reactivate and ID-preservation scenarios are exercised.
6. Failure-injection/compensation is exercised.
7. The matrix is updated from `RUNTIME PENDING` to `PASS` only when direct evidence exists.
