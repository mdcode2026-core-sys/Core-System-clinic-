...
import { Plus, RefreshCw, ArrowRight, CheckCircle2 } from "lucide-react";
...
export function TreatmentPlanWorkspace() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6" dir="rtl">
      ...
      {showCreate && <Card><CardHeader><CardTitle>إنشاء خطة علاج</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
        <input className="rounded-md border p-3" placeholder="عنوان الخطة" value={title} onChange={(e) => setTitle(e.target.value)} />
        <input className="rounded-md border p-3" placeholder="ملخص التشخيص" value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} />
        <textarea className="min-h-24 rounded-md border p-3 md:col-span-2" placeholder="الأهداف العلاجية" value={goals} onChange={(e) => setGoals(e.target.value)} />
        <div className="flex gap-2 md:col-span-2"><Button onClick={() => void create()} disabled={!title.trim()}>إنشاء</Button><Button variant="outline" onClick={() => setShowCreate(false)}>إلغاء</Button></div>
      </CardContent></Card>}
      ...
      {patientId && <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
        ...
        {selected ? <div className="space-y-4">
          ...
          <Card><CardHeader><CardTitle>الأنشطة المخططة ({selected.items.length})</CardTitle></CardHeader><CardContent className="space-y-3"><div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]"><input className="rounded-md border p-3" placeholder="اسم النشاط / الجلسة" value={itemTitle} onChange={(e) => setItemTitle(e.target.value)} /><input className="rounded-md border p-3" placeholder="وصف مختصر" value={itemDescription} onChange={(e) => setItemDescription(e.target.value)} /><Button onClick={() => void addItem()} disabled={!itemTitle.trim()}><Plus className="ml-2 h-4 w-4" /> إضافة</Button></div>{selected.items.length === 0 ? <p className="py-4 text-center text-sm text-muted-foreground">لا توجد أنشطة مخططة بعد.</p> : selected.items.map((item) => <div key={item.id} className="rounded-lg border p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><span className="font-medium">{item.sequence_no}. {item.title}</span>{item.description && <p className="text-sm text-muted-foreground">{item.description}</p>}</div><select className="rounded-md border p-2 text-sm" value={item.status} onChange={(e) => void run(() => updateTreatmentPlanItem(item.id, { status: e.target.value as any }), selected.id)}>{Object.entries(itemStatusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div></div>)}</CardContent></Card>
