...
type Surface = "billing" | "inventory";
...
export function FinancialResourcesCenter({ surface, locale, patients, plans, insurance, suppliers, purchaseOrders, inventoryItems, permissions, currency }: Props) {
  ...
  const showBilling = surface === "billing";
  const showInventory = surface === "inventory";
  ...
  return <div className="space-y-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    {showBilling && <>
      ...
      {canPlan && <Card><CardHeader><CardTitle>{t.newPlan}</CardTitle></CardHeader><CardContent><form onSubmit={submitPlan} className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <div><Label>{t.patient}</Label><select className="mt-1 w-full rounded-md border p-2" value={patientId} onChange={(e) => setPatientId(e.target.value)} required>{patients.map((p) => <option key={p.id} value={p.id}>{patientName[p.id]}</option>)}</select></div>
