...
type PurchaseOrder = { id: string; order_number: string | null; order_date: string; status: string; total_subunits: number; supplier: { id: string; name: string } | null };
type InventoryItem = { id: string; name: string; current_stock: number; unit: string };
type Plan = { id: string; patient_id: string; total_amount_subunits: number; patient_responsibility_subunits: number; insurance_covered_subunits: number; status: string; installments: { id: string; installment_no: number; due_date: string; amount_subunits: number; amount_paid_subunits: number; status: string }[] };
type Insurance = { id: string; patient_id: string; payer_name: string; policy_number: string | null; member_number: string | null; claim_ready: boolean; status: string; reconciliation_status: string };
...
export function FinancialResourcesCenter({ locale, patients, plans, insurance, suppliers, purchaseOrders, inventoryItems, permissions, currency }: Props) {
  ...
  const patientName = useMemo(() => Object.fromEntries(patients.map((p) => [p.id, `${p.first_name} ${p.last_name}`])), [patients]);
  ...
  async function submitPlan(e: React.FormEvent) {
    ...
    const totalSubunits = Math.round(Number(total) * 100); const insuranceSubunits = Math.round(Number(insuranceCovered) * 100); const count = Math.max(1, Number.parseInt(installmentCount, 10) || 1);
    const responsibility = totalSubunits - insuranceSubunits;
    const today = new Date();
    const per = Math.floor(responsibility / count); const remainder = responsibility - per * count;
    ...
    setBusy(false); setMessage(result.success ? t.success : result.error);
    if (result.success) { setTotal(""); setInsuranceCovered("0"); window.location.reload(); }
  }
  ...
  async function submitInsurance(e: React.FormEvent) {
    ...
    setBusy(false); setMessage(result.success ? t.success : result.error);
    if (result.success) { setPayerName(""); setPolicyNumber(""); setMemberNumber(""); window.location.reload(); }
  }
  ...
  async function submitSupplier(e: React.FormEvent) {
    ...
    setBusy(false); setMessage(result.success ? t.success : result.error);
    if (result.success) { setSupplierName(""); setSupplierPhone(""); setSupplierEmail(""); window.location.reload(); }
  }
  ...
  return <div className="space-y-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    {canPlan && <Card><CardHeader><CardTitle>{t.newPlan}</CardTitle></CardHeader><CardContent><form onSubmit={submitPlan} className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
      <div><Label>{t.patient}</Label><select className="mt-1 w-full rounded-md border p-2" value={patientId} onChange={(e) => setPatientId(e.target.value)}>{patients.map((p) => <option key={p.id} value={p.id}>{patientName[p.id]}</option>)}</select></div>
      ...
      <div><Label>{t.insuranceCovered}</Label><Input className="mt-1" type="number" min="0" step="0.01" value={insuranceCovered} onChange={(e) => setInsuranceCovered(e.target.value)} /></div>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>{t.financialPlans}</CardTitle></CardHeader><CardContent className="space-y-3">{plans.length === 0 ? <p className="text-sm text-muted-foreground">{t.noPlans}</p> : plans.slice(0, 10).map((p) => <div key={p.id} className="rounded-lg border p-3"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{patientName[p.patient_id] ?? p.patient_id}</p><p className="text-sm text-muted-foreground">{money(p.total_amount_subunits)} · {money(p.patient_responsibility_subunits)} {t.patientResponsibility}</p></div><Badge>{p.status}</Badge></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{p.installments.map((i) => <div key={i.id} className="rounded-md bg-muted/40 p-2 text-sm"><div className="flex justify-between"><span>{t.installments} #{i.installment_no}</span><span>{money(i.amount_subunits)}</span></div><div className="text-muted-foreground">{i.due_date} · {i.status}</div></div>)}</div><Link className="mt-3 inline-block text-sm underline" href={`/patients/${p.patient_id}`}>{t.openPatient}</Link></div>)}</CardContent></Card>

      <Card><CardHeader><CardTitle>{t.insurance}</CardTitle></CardHeader><CardContent className="space-y-4">{canInsurance && <form onSubmit={submitInsurance} className="grid gap-3 md:grid-cols-2"><select className="rounded-md border p-2" value={patientId} onChange={(e) => setPatientId(e.target.value)}>{patients.map((p) => <option key={p.id} value={p.id}>{patientName[p.id]}</option>)}</select><Input placeholder={t.payerName} value={payerName} onChange={(e) => setPayerName(e.target.value)} required /><Input placeholder={t.policyNumber} value={policyNumber} onChange={(e) => setPolicyNumber(e.target.value)} /><Input placeholder={t.memberNumber} value={memberNumber} onChange={(e) => setMemberNumber(e.target.value)} /><Button disabled={busy}>{busy ? t.loading : t.create}</Button></form>}{insurance.length === 0 ? <p className="text-sm text-muted-foreground">{t.noInsurance}</p> : insurance.slice(0, 10).map((x) => <div key={x.id} className="flex items-center justify-between rounded-lg border p-3"><div><p className="font-medium">{patientName[x.patient_id] ?? x.patient_id}</p><p className="text-sm text-muted-foreground">{x.payer_name}{x.policy_number ? ` · ${x.policy_number}` : ""}</p></div><Badge>{x.claim_ready ? t.claimReady : x.reconciliation_status}</Badge></div>)}</CardContent></Card>
    </div>

    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>{t.suppliers}</CardTitle></CardHeader><CardContent className="space-y-4">{canPurchasing && <form onSubmit={submitSupplier} className="grid gap-3 md:grid-cols-2"><Input placeholder={t.supplierName} value={supplierName} onChange={(e) => setSupplierName(e.target.value)} required /><Input placeholder={t.phone} value={supplierPhone} onChange={(e) => setSupplierPhone(e.target.value)} /><Input placeholder={t.email} value={supplierEmail} onChange={(e) => setSupplierEmail(e.target.value)} /><Button disabled={busy}>{busy ? t.loading : t.create}</Button></form>}{suppliers.length === 0 ? <p className="text-sm text-muted-foreground">{t.noSuppliers}</p> : suppliers.slice(0, 10).map((s) => <div key={s.id} className="rounded-lg border p-3"><p className="font-medium">{s.name}</p><p className="text-sm text-muted-foreground">{s.phone ?? ""}{s.email ? ` · ${s.email}` : ""}</p></div>)}</CardContent></Card>

      <Card><CardHeader><CardTitle>{t.purchasing}</CardTitle></CardHeader><CardContent className="space-y-4">{canPurchasing && suppliers.length > 0 && inventoryItems.length > 0 && <form onSubmit={submitPurchaseOrder} className="grid gap-3 md:grid-cols-2"><select className="rounded-md border p-2" value={supplierId} onChange={(e) => setSupplierId(e.target.value)}>{suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><select className="rounded-md border p-2" value={inventoryItemId} onChange={(e) => setInventoryItemId(e.target.value)}>{inventoryItems.map((i) => <option key={i.id} value={i.id}>{i.name} ({i.current_stock} {i.unit})</option>)}</select><Input type="number" min="1" step="1" value={purchaseQuantity} onChange={(e) => setPurchaseQuantity(e.target.value)} placeholder={t.quantity} /><Input type="number" min="0" step="0.01" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} placeholder={t.unitCost} /><Button disabled={busy}>{busy ? t.loading : t.create}</Button></form>}{purchaseOrders.length === 0 ? <p className="text-sm text-muted-foreground">{t.noOrders}</p> : purchaseOrders.slice(0, 10).map((o) => <div key={o.id} className="flex items-center justify-between rounded-lg border p-3"><div><p className="font-medium">{o.order_number ?? o.id.slice(0, 8)} · {o.supplier?.name ?? ""}</p><p className="text-sm text-muted-foreground">{o.order_date} · {money(o.total_subunits)}</p></div><Badge>{o.status}</Badge></div>)}</CardContent></Card>
    </div>
