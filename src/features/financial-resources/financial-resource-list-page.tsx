...
const config: Record<Resource, { title: string; permission: string; table: string; columns: FinancialResourceColumn[]; select: string }> = {
  ...
  consumption: { title: "Consumption", permission: "inventory:read", table: "inventory_ledger", select: "id, inventory_item_id, quantity, transaction_type, reason, created_at", columns: [{ key: "inventory_item_id", label: "Item" }, { key: "quantity", label: "Quantity" }, { key: "transaction_type", label: "Transaction" }, { key: "reason", label: "Reason" }, { key: "created_at", label: "Date" }] },
