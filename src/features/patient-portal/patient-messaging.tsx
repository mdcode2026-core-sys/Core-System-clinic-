...
export function PatientMessaging() {
  ...
  async function load() { try { setMessages(await listPatientMessages()); } catch (e) { setMessage(e instanceof Error ? e.message : "Messaging is unavailable"); } }
  useEffect(() => { void load(); }, []);
  async function send() { setBusy(true); setMessage(""); try { await sendPatientMessage(body); setBody(""); await load(); } catch (e) { setMessage(e instanceof Error ? e.message : "Unable to send message"); } finally { setBusy(false); } }
