...
export function CreateRoleDialog({ open, onClose, onSuccess }: CreateRoleDialogProps) {
  ...
  const handleSave = async () => {
    ...
    const result = await createRole({
      role_key: roleKey,
      role_name: roleName,
      role_name_ar: roleNameAr || undefined,
    ...
    if (result.success) {
      ...
      setSaveError(result.error || "Failed to create role");
    }
  };

  const canSave =
    roleKey.trim().length >= 2 &&
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle>إنشاء دور مخصص جديد</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="role-key">المعرف الفني (role_key)</Label>
                <Input
                  id="role-key"
                  value={roleKey}
                  onChange={(e) => setRoleKey(e.target.value)}
                  placeholder="مثال: senior_doctor"
                  disabled={isSaving}
                />
                <p className="text-xs text-muted-foreground">
                  حروف إنجليزية صغيرة وأرقام وشرطات سفلية فقط
                </p>
              </div>
              <div className="space-y-1.5">
