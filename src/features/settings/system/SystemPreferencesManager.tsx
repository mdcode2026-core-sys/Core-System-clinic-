...
export function SystemPreferencesManager() {
  ...
  const handleChange = useCallback((field: "language" | "direction" | "timezone" | "currency", value: string) => {
    setLocalPrefs((prev) => {
      const base = prev ?? preferences;
      return base ? { ...base, [field]: value } as typeof base : prev;
    });
  }, [preferences]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const result = await updateSystemPreferences({ language: current.language, direction: current.direction, timezone: current.timezone, currency: current.currency });
    setSaving(false);
    if (result.success) {
  ...
  const languageLabel = (value: SupportedLanguage) => t.languages[value];
  const directionLabel = (value: SupportedDirection) => t.directions[value];
  ...
  return <div className="space-y-6" dir={direction}>
    ...
    <Card><CardHeader><CardTitle className="text-base">{t.generalSettings}</CardTitle></CardHeader><CardContent className="space-y-6">
      <div className="space-y-2"><Label htmlFor="language">{t.language}</Label><Select value={current.language} onValueChange={(v) => handleChange("language", v)} disabled={!canUpdate || saving}><SelectTrigger id="language" className="w-full sm:w-[280px]"><SelectValue placeholder={t.selectLanguage} /></SelectTrigger><SelectContent>{SUPPORTED_LANGUAGES.map((lang) => <SelectItem key={lang.value} value={lang.value}>{languageLabel(lang.value)}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">{t.languageHint}</p></div>
      <div className="space-y-2"><Label htmlFor="direction">{t.direction}</Label><Select value={current.direction} onValueChange={(v) => handleChange("direction", v)} disabled={!canUpdate || saving}><SelectTrigger id="direction" className="w-full sm:w-[280px]"><SelectValue placeholder={t.selectDirection} /></SelectTrigger><SelectContent>{SUPPORTED_DIRECTIONS.map((item) => <SelectItem key={item.value} value={item.value}>{directionLabel(item.value)}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">{t.directionHint}</p></div>
