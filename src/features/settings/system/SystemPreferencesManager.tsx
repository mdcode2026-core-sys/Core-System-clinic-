...
export function SystemPreferencesManager() {
  ...
  return <div className="space-y-6" dir={direction}>
    ...
    <Card><CardHeader><CardTitle className="text-base">{t.generalSettings}</CardTitle></CardHeader><CardContent className="space-y-6">
      ...
      <div className="space-y-2"><Label htmlFor="direction">{t.direction}</Label><Select value={current.direction} onValueChange={(v) => handleChange("direction", v)} disabled={!canUpdate || saving}><SelectTrigger id="direction" className="w-full sm:w-[280px]"><SelectValue placeholder={t.selectDirection} /></SelectTrigger><SelectContent>{SUPPORTED_DIRECTIONS.map((value) => <SelectItem key={value} value={value}>{t.directions[value]}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">{t.directionHint}</p></div>
