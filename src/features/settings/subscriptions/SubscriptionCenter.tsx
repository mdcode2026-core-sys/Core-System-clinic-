...
export function SubscriptionCenter() {
  ...
  const applyCode = async () => {
    if (!code.trim()) return;
    setCodeState("loading"); setCodeMessage(null);
    const result = await applyActivationCode(code.trim());
    const message = result.error || a.subscription.notImplemented;
    setCodeState(result.success ? "success" : "error");
    setCodeMessage(message);
    if (result.success) toast.success(a.subscription.apply); else toast.error(message);
  };
  ...
  return <div className="space-y-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><KeyRound className="h-5 w-5" />{a.subscription.activationCode}</CardTitle><CardDescription>{s.activationDescription}</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row"><Input placeholder={s.activationPlaceholder} value={code} onChange={e => { setCode(e.target.value); setCodeState("idle"); }} disabled={codeState === "loading"} /><Button onClick={() => void applyCode()} disabled={!code.trim() || codeState === "loading"}>{codeState === "loading" && <Loader2 className="me-2 h-4 w-4 animate-spin" />}{a.subscription.apply}</Button></div>{codeMessage && <div className={`flex items-center gap-2 text-sm ${codeState === "success" ? "text-green-600" : "text-destructive"}`}>{codeState === "success" ? <CheckCircle className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}{codeMessage}</div>}<div className="rounded-lg border p-3 text-sm"><p className="font-medium">{s.note}</p><p className="text-muted-foreground">{a.subscription.notImplemented}</p></div></CardContent></Card>
    <Card><CardHeader><CardTitle>{s.contactInformation}</CardTitle></CardHeader><CardContent className="space-y-2"><a href={`tel:${TEMPORARY_CONTACT.phone}`} className="flex items-center gap-3 rounded-lg p-3 hover:bg-muted"><Phone className="h-5 w-5" />{TEMPORARY_CONTACT.phone}</a><a href={`https://wa.me/${TEMPORARY_CONTACT.whatsapp.replace(/^\+/, "")}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-lg p-3 hover:bg-muted"><MessageCircle className="h-5 w-5" />{TEMPORARY_CONTACT.whatsapp}</a><a href={`mailto:${TEMPORARY_CONTACT.email}`} className="flex items-center gap-3 rounded-lg p-3 hover:bg-muted"><Mail className="h-5 w-5" />{TEMPORARY_CONTACT.email}</a></CardContent></Card>
  </div>;
}
