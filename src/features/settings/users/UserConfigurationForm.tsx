...
export function UserConfigurationForm({ user, roles, onClose, onSaved, onError }: Props) {
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!user && password.length < 8) { onError("PASSWORD_REQUIRED_MIN_8"); return; }
    if (user && password !== "" && password.length < 8) { onError("PASSWORD_MIN_8"); return; }
    setSaving(true);
    ...
    const result = user
      ? await updateClinicUser({ id: user.id, full_name: fullName, email, phone, password: password || undefined, role_id: roleId, workspace, is_active: isActive, directPermissionIds, revokedPermissionIds })
      : await createClinicUser({ full_name: fullName, email, phone, password, role_id: roleId, workspace, is_active: isActive, directPermissionIds, revokedPermissionIds });
    setSaving(false);
  ...
  return <form onSubmit={save} className="space-y-6" dir={ar ? "rtl" : "ltr"}>
    <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{user ? (ar ? "تعديل المستخدم" : "Edit user") : (ar ? "إعداد مستخدم جديد" : "Configure new user")}</h2><p className="mt-1 text-sm text-muted-foreground">{ar ? "كل إعدادات المستخدم الأساسية والوصول والدخول في نموذج واحد ثم حفظ واحد." : "Configure identity, role, workspace, access, account status, and password in one form with one Save action."}</p></div><Button type="button" variant="ghost" size="sm" onClick={onClose}><X className="me-1 h-4 w-4" />{ar ? "إلغاء" : "Cancel"}</Button></div>
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><KeyRound className="h-4 w-4" />{ar ? "4. حالة الحساب وكلمة المرور" : "4. Account status & password"}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>{ar ? "حالة الحساب" : "Account status"}</Label><Select value={isActive ? "active" : "inactive"} onValueChange={value => setIsActive(value === "active")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">{ar ? "نشط" : "Active"}</SelectItem><SelectItem value="inactive">{ar ? "غير نشط" : "Not active"}</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label htmlFor="user-password">{user ? (ar ? "كلمة المرور الجديدة (اختياري)" : "New password (optional)") : (ar ? "كلمة المرور" : "Password")}</Label><Input id="user-password" type="password" value={password} onChange={e => setPassword(e.target.value)} minLength={8} required={!user} autoComplete={user ? "new-password" : "new-password"} placeholder={user ? (ar ? "اتركها فارغة للإبقاء على الحالية" : "Leave blank to keep current") : undefined} /></div><div className="md:col-span-2 rounded-lg border bg-muted/30 p-4"><p className="text-sm font-medium">{ar ? "طريقة الدخول" : "Login method"}</p><p className="mt-1 text-sm text-muted-foreground">{ar ? "يتم إنشاء كلمة المرور هنا ولا تُحفظ في قاعدة بيانات CORE SYSTEM. يستخدم الحساب Supabase Auth للدخول. حالة الحساب تحدد ما إذا كان المستخدم مسموحًا له بالدخول." : "The password is set here and is never stored in the CORE SYSTEM database. Supabase Auth manages authentication. Account status determines whether the user is allowed to sign in."}</p></div></CardContent></Card>
    <Separator /><div className="flex items-center justify-end gap-3"><Button type="button" variant="outline" onClick={onClose}>{ar ? "إلغاء" : "Cancel"}</Button><Button type="submit" disabled={saving || !roleId || !workspace || !fullName.trim() || !email.trim() || (!user && password.length < 8)}>{saving ? <Loader2 className="me-2 h-4 w-4 animate-spin" /> : <Save className="me-2 h-4 w-4" />}{saving ? (ar ? "جاري الحفظ..." : "Saving...") : (ar ? "حفظ إعدادات المستخدم" : "Save user configuration")}</Button></div>
  </form>;
}