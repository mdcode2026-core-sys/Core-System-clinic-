...
import { AlertTriangle, Check, KeyRound, Loader2, Minus, Plus, Save, ShieldCheck, X } from "lucide-react";
...
export function UserConfigurationForm({ user, roles, onClose, onSaved, onError }: Props) {
  ...
  const [status, setStatus] = useState(user?.is_active ?? true);
  ...
  return (
    <form onSubmit={save} className="space-y-6" dir={ar ? "rtl" : "ltr"}>
      <div className="flex items-start justify-between gap-4">
        <div><h2 className="text-xl font-semibold">{user ? (ar ? "تعديل المستخدم" : "Edit user") : (ar ? "إعداد مستخدم جديد" : "Configure new user")}</h2><p className="mt-1 text-sm text-muted-foreground">{ar ? "كل إعدادات المستخدم الأساسية والوصول والدخول في نموذج واحد ثم حفظ واحد." : "Configure identity, role, workspace, access, and login in one form with one Save action."}</p></div>
        <Button type="button" variant="ghost" size="sm" onClick={onClose}><X className="me-1 h-4 w-4" />{ar ? "إلغاء" : "Cancel"}</Button>
      </div>

      <Card><CardHeader><CardTitle className="text-base">{ar ? "1. البيانات الأساسية" : "1. Basic information"}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="user-full-name">{ar ? "الاسم الكامل" : "Full name"}</Label><Input id="user-full-name" value={fullName} onChange={e => setFullName(e.target.value)} required /></div>
        <div className="space-y-2"><Label htmlFor="user-email">{ar ? "البريد الإلكتروني" : "Email"}</Label><Input id="user-email" type="email" value={email} onChange={e => setEmail(e.target.value)} required disabled={user?.auth_user_id === undefined ? false : false} /></div>
        <div className="space-y-2"><Label htmlFor="user-phone">{ar ? "رقم الهاتف" : "Phone"}</Label><Input id="user-phone" value={phone} onChange={e => setPhone(e.target.value)} /></div>
        {user && <div className="space-y-2"><Label>{ar ? "حالة الحساب" : "Account status"}</Label><Select value={status ? "active" : "inactive"} onValueChange={value => setStatus(value === "active")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="active">{ar ? "نشط" : "Active"}</SelectItem><SelectItem value="inactive">{ar ? "غير نشط" : "Inactive"}</SelectItem></SelectContent></Select></div>}
      </CardContent></Card>

      <Card><CardHeader><CardTitle className="text-base">{ar ? "2. الدور ومساحة العمل" : "2. Role & workspace"}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2"><Label>{ar ? "الدور" : "Role"}</Label><Select value={roleId} onValueChange={value => { setRoleId(value); const role = roles.find(item => item.id === value); if (role?.workspace) setWorkspace(role.workspace); }}><SelectTrigger><SelectValue placeholder={ar ? "اختر الدور" : "Choose role"} /></SelectTrigger><SelectContent>{roles.filter(role => role.role_key !== "super_admin").map(role => <SelectItem key={role.id} value={role.id}>{ar ? role.role_name_ar || role.role_name : role.role_name}</SelectItem>)}</SelectContent></Select>{selectedRole?.workspace && <p className="text-xs text-muted-foreground">{ar ? `المساحة المقترحة: ${selectedRole.workspace}` : `Suggested workspace: ${selectedRole.workspace}`}</p>}</div>
        <div className="space-y-2"><Label>{ar ? "مساحة العمل" : "Workspace"}</Label><Select value={workspace} onValueChange={value => setWorkspace(value as UserWorkspace)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{workspaces.map(item => <SelectItem key={item.value} value={item.value}>{ar ? item.ar : item.en}</SelectItem>)}</SelectContent></Select></div>
      </CardContent></Card>

      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><ShieldCheck className="h-4 w-4" />{ar ? "3. الصلاحيات والاستثناءات" : "3. Permissions & exceptions"}</CardTitle></CardHeader><CardContent className="space-y-5">
        <p className="text-sm text-muted-foreground">{ar ? "الوضع الافتراضي يأتي من الدور. المنح المباشرة والاستثناءات تُحفظ مع نفس المستخدم ضمن نفس عملية الحفظ." : "The role provides the default access. Direct grants and explicit exceptions are saved with the same user in the same Save operation."}</p>
        {catalogLoading || (user && (directLoading || overridesLoading)) ? <div className="flex items-center gap-2 py-6 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{ar ? "جاري تحميل الصلاحيات..." : "Loading access catalog..."}</div> : <div className="max-h-[420px] space-y-4 overflow-y-auto pe-1">{Object.entries(groupedCatalog).map(([resource, permissions]) => <div key={resource} className="rounded-lg border p-3"><div className="mb-2 flex items-center justify-between"><span className="font-medium">{resource}</span><Badge variant="outline">{permissions.length}</Badge></div><div className="grid gap-2 md:grid-cols-2">{permissions.map(permission => { const state = access[permission.id] ?? "default"; return <button key={permission.id} type="button" onClick={() => togglePermission(permission.id)} className={`flex items-center gap-2 rounded-md border p-2 text-start ${state === "granted" ? "border-green-300 bg-green-50" : state === "revoked" ? "border-red-300 bg-red-50" : "hover:bg-muted/50"}`}>{state === "granted" ? <Plus className="h-4 w-4" /> : state === "revoked" ? <Minus className="h-4 w-4" /> : <Check className="h-4 w-4 opacity-30" />}<span className="text-sm">{permission.permission_name || permission.action}</span><span className="ms-auto text-xs text-muted-foreground">{state}</span></button>; })}</div></div>)}</div>}
      </CardContent></Card>

      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><KeyRound className="h-4 w-4" />{ar ? "4. الحساب والدعوة" : "4. Account & invitation"}</CardTitle></CardHeader><CardContent><div className="rounded-lg border bg-muted/30 p-4"><p className="text-sm font-medium">{ar ? "كلمة المرور" : "Password"}</p><p className="mt-1 text-sm text-muted-foreground">{ar ? "تتم إدارة كلمة المرور عبر Supabase Auth. لا ينشئ Clinic Admin كلمة المرور ولا يعرفها." : "Password is managed by Supabase Auth. The Clinic Admin does not create or know the password."}</p></div>{!user && <p className="mt-3 text-xs text-muted-foreground">{ar ? "عند الحفظ سيتم إنشاء حساب الدخول وإرسال دعوة التفعيل." : "Saving creates the login account and sends the activation invitation."}</p>}{user?.auth_user_id && <p className="mt-3 text-xs text-muted-foreground">{ar ? "يمكن إعادة إرسال/إعادة تفعيل الدخول من قائمة المستخدمين عند الحاجة." : "Invitation/reactivation actions remain available from the Users list when needed."}</p>}</CardContent></Card>
