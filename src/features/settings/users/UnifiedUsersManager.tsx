...
export function UnifiedUsersManager() {
  ...
  async function activate(userId: string) { setActionError(null); setActivating(userId); const result = await activateClinicUserAccount(userId); setActivating(null); if (!result.success) setActionError(errorLabel(result.error)); else { setActivationLink(result.activationLink ?? null); void refetch(); } }
  ...
  return <div className="space-y-6" dir={ar ? "rtl" : "ltr"}>
    ...
    <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>{ar ? "المستخدم" : "User"}</TableHead><TableHead>{a.users.role}</TableHead><TableHead>{ar ? "مساحة العمل" : "Workspace"}</TableHead><TableHead>{ar ? "الدخول" : "Login"}</TableHead><TableHead>{a.users.status}</TableHead><TableHead>{a.users.actions}</TableHead></TableRow></TableHeader><TableBody>{users.map(user => <TableRow key={user.id} className={!user.is_active ? "opacity-60" : ""}><TableCell><div className="flex flex-col gap-0.5"><span className="font-medium">{user.full_name || "—"}</span><span className="flex items-center gap-1 text-xs text-muted-foreground"><Mail className="h-3 w-3" />{user.email || "—"}</span>{user.phone && <span className="flex items-center gap-1 text-xs text-muted-foreground"><Phone className="h-3 w-3" />{user.phone}</span>}</div></TableCell><TableCell>{ar ? user.role_name_ar || user.role_name || user.role : user.role_name || user.role}</TableCell><TableCell><span className="inline-flex items-center gap-1.5 text-sm"><BriefcaseBusiness className="h-4 w-4 text-muted-foreground" />{workspaceLabel(user.role_workspace, ar)}</span></TableCell><TableCell>{user.auth_user_id ? <Badge>{ar ? "مرتبط" : "Linked"}</Badge> : <Badge variant="outline">{ar ? "بانتظار التفعيل" : "Invitation pending"}</Badge>}</TableCell><TableCell><Badge variant={user.is_active ? "default" : "secondary"}>{user.is_active ? a.users.active : a.users.inactive}</Badge></TableCell><TableCell><div className="flex flex-wrap items-center gap-2">{canUpdate && <Button variant="ghost" size="sm" onClick={() => { setEditingUser(user); setActionError(null); setFormOpen(true); }}><Pencil className="h-4 w-4" /></Button>}{canUpdate && !user.auth_user_id && <Button variant="outline" size="sm" disabled={activating === user.id} onClick={() => void activate(user.id)}><KeyRound className="me-1 h-4 w-4" />{ar ? "إرسال التفعيل" : "Send invitation"}</Button>}{canDelete && user.auth_user_id !== user.id && <div className="flex items-center gap-2"><Switch checked={user.is_active} onCheckedChange={async checked => { const result = await toggleClinicUserActive(user.id, checked); if (!result.success) setActionError(errorLabel(result.error)); else void refetch(); }} /><span className="text-xs text-muted-foreground">{user.is_active ? a.users.active : a.users.inactive}</span></div>}</div></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
