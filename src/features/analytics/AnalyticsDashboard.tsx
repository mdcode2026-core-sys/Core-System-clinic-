...
export function AnalyticsDashboard() {
  ...
  return <div className="space-y-8" dir={direction}><h1 className="text-3xl font-bold">{t.title}</h1>{groups.map(([key, label, prefix]) => { const items = kpis.filter((k) => k.id.startsWith(prefix)); if (!items.length) return null; return <section key={key}><h2 className="mb-3 text-lg font-semibold">{label}</h2><KpiGrid kpis={items} /></section>; })}</div>;
}
