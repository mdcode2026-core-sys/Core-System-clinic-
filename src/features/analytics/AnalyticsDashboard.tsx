...
export function AnalyticsDashboard() {
  ...
  const { data: kpis, isLoading: kpiLoading, error } = useAnalyticsOverview(user?.id, preset, customFrom || undefined, customTo || undefined, customReady);
  const comparison = useAnalyticsComparison(user?.id, "revenue.total", preset, customFrom || undefined, customTo || undefined, customReady);
  const trend = useAnalyticsTrend(user?.id, "revenue.total", preset, customFrom || undefined, customTo || undefined, customReady);
  ...
  if (authLoading || (customReady && kpiLoading)) return <div className="space-y-6" dir={direction}><h1 className="text-3xl font-bold">{t.title}</h1><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-24 animate-pulse rounded-lg bg-muted" />)}</div></div>;
  ...
  if (preset === "custom" && !customReady) return <div className="space-y-6" dir={direction}><h1 className="text-3xl font-bold">{t.title}</h1><div className="rounded-lg border p-4 text-muted-foreground">{locale === "ar" ? "حدد تاريخ البداية والنهاية لعرض التحليلات." : "Select both start and end dates to load analytics."}</div></div>;
  ...
  const groups = [["patients", t.sections.patients, "patients."], ["appointments", t.sections.appointments, "appointments."], ["queue", t.sections.queue, "queue."], ["revenue", t.sections.revenue, "revenue."], ["invoices", t.sections.invoices, "invoices."], ["inventory", t.sections.inventory, "inventory."], ["followup", t.sections.followup, "followup."], ["workforce", locale === "ar" ? "القوى العاملة" : "Workforce", "workforce."], ["communications", locale === "ar" ? "الاتصالات" : "Communications", "communications."], ["coordination", locale === "ar" ? "التنسيق التشغيلي" : "Operational Coordination", "coordination."]] as const;
  ...
  return <div className="space-y-8" dir={direction}>
    <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 md:flex-row md:items-end md:justify-between">
      ...
      <div className="flex flex-wrap items-end gap-2"><label className="text-sm"><span className="mb-1 block text-muted-foreground">{locale === "ar" ? "الفترة" : "Period"}</span><select className="h-9 rounded-md border bg-background px-3" value={preset} onChange={(e) => setPreset(e.target.value as DatePreset)}>{presets.map((p) => <option key={p.value} value={p.value}>{locale === "ar" ? p.ar : p.en}</option>)}</select></label>{preset === "custom" ? <><label className="text-sm"><span className="mb-1 block text-muted-foreground">{locale === "ar" ? "من" : "From"}</span><input className="h-9 rounded-md border bg-background px-3" type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} /></label><label className="text-sm"><span className="mb-1 block text-muted-foreground">{locale === "ar" ? "إلى" : "To"}</span><input className="h-9 rounded-md border bg-background px-3" type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} /></label></> : null}</div>
    </div>
