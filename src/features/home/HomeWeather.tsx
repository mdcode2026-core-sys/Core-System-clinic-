...
export async function HomeWeather({ address, countryCode, isArabic }: HomeWeatherProps) {
  ...
  const units = isArabic ? { temperature: "°م", wind: "كم/س" } : { temperature: "°C", wind: "km/h" };
  return <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2 py-1" aria-label={isArabic ? `الطقس في ${weather.location}` : `Weather in ${weather.location}`}>
    ...
    <div className="min-w-0"><div className="flex items-baseline gap-2"><span className="text-2xl font-bold tabular-nums text-[var(--cs-ink-950)]">{Math.round(weather.temperature)}{units.temperature}</span><span className="text-sm font-medium text-[var(--cs-slate-700)]">{weatherText(weather.code, isArabic)}</span></div><p className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[var(--cs-slate-500)]"><span>{weather.location}</span>{typeof weather.humidity === "number" ? <span className="inline-flex items-center gap-1"><Droplets className="h-3 w-3" aria-hidden="true" />{weather.humidity}%</span> : null}{typeof weather.wind === "number" ? <span className="inline-flex items-center gap-1"><Wind className="h-3 w-3" aria-hidden="true" />{Math.round(weather.wind)} {units.wind}</span> : null}</p></div>
  </div>;
}
