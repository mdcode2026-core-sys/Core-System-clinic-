...
export function NotificationsHeaderControl({ isArabic }: NotificationsHeaderControlProps) {
  ...
  return <div className="relative">
    <button type="button" onClick={() => setOpen((value) => !value)} className="relative inline-flex h-10 w-10 items-center justify-center rounded-lg text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 md:w-auto md:px-2.5" aria-label={isArabic ? unreadCount > 0 ? `الإشعارات، ${unreadCount} غير مقروء` : "الإشعارات" : unreadCount > 0 ? `Notifications, ${unreadCount} unread` : "Notifications"} aria-expanded={open} aria-haspopup="dialog" data-testid="notifications-header-control">
      <Bell className="h-5 w-5" aria-hidden="true" /><span className="sr-only">{isArabic ? "الإشعارات" : "Notifications"}</span>
      {unreadCount > 0 && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-red-600 px-1 text-center text-[10px] font-semibold leading-4 text-white" aria-label={isArabic ? `${unreadCount} غير مقروء` : `${unreadCount} unread`}>{unreadCount > 99 ? "99+" : unreadCount}</span>}
      <span className="hidden xl:inline ms-1 text-sm font-medium">{isArabic ? "الإشعارات" : "Notifications"}</span>
    </button>
    {open && <section role="dialog" aria-label={isArabic ? "الإشعارات" : "Notifications"} className={cn("absolute top-12 z-50 w-[min(23rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border bg-white shadow-xl", isArabic ? "left-0" : "right-0")} data-testid="notifications-header-panel">
      <div className="flex items-center justify-between border-b px-4 py-3"><div><h2 className="font-semibold text-gray-900">{isArabic ? "الإشعارات" : "Notifications"}</h2><p className="text-xs text-gray-500">{isArabic ? unreadCount > 0 ? `${unreadCount} غير مقروءة` : "لا توجد إشعارات غير مقروءة" : unreadCount > 0 ? `${unreadCount} unread` : "No unread notifications"}</p></div>{unreadCount > 0 && <button type="button" onClick={() => void handleReadAll()} className="rounded-md px-2 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">{isArabic ? "تعليم الكل كمقروء" : "Mark all read"}</button>}</div>
      <div className="max-h-[min(28rem,70vh)] overflow-y-auto">
        {isLoading ? <p className="px-4 py-8 text-center text-sm text-gray-500">{isArabic ? "جارٍ التحميل..." : "Loading..."}</p> : isError ? <p className="px-4 py-8 text-center text-sm text-red-600">{isArabic ? "تعذر تحميل الإشعارات." : "Unable to load notifications."}</p> : data?.items.length ? data.items.map((item) => <button key={item.id} type="button" onClick={() => void handleRead(item.id)} className="block w-full border-b px-4 py-3 text-start transition-colors last:border-b-0 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500"><div className="flex gap-3"><span className={cn("mt-1.5 h-2 w-2 shrink-0 rounded-full", item.is_read ? "bg-gray-300" : "bg-blue-600")} aria-hidden="true" /><div className="min-w-0 flex-1"><p className={cn("text-sm text-gray-900", item.is_read ? "font-normal" : "font-semibold")}>{item.title}</p>{item.message && <p className="mt-1 line-clamp-2 text-xs text-gray-600">{item.message}</p>}<p className="mt-1 text-[11px] text-gray-400">{formatTime(item.created_at, isArabic)}</p></div></div></button>) : <p className="px-4 py-8 text-center text-sm text-gray-500">{isArabic ? "لا توجد إشعارات حالياً." : "No notifications yet."}</p>}
      </div>
    </section>}
  </div>;
}
