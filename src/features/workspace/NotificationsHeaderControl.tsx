...
export function NotificationsHeaderControl({ isArabic }: NotificationsHeaderControlProps) {
  ...
  return <div className="relative">
    <button ref={triggerRef} type="button" onClick={() => setOpen(value => !value)} className="relative inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 md:w-auto md:px-2.5" aria-label={isArabic ? unreadCount > 0 ? `الإشعارات، ${unreadCount} غير مقروء` : "الإشعارات" : unreadCount > 0 ? `Notifications, ${unreadCount} unread` : "Notifications"} aria-expanded={open} aria-controls="notifications-header-panel" aria-haspopup="dialog" data-testid="notifications-header-control">
      <Bell className="h-[18px] w-[18px]" aria-hidden="true" /><span className="sr-only">{isArabic ? "الإشعارات" : "Notifications"}</span>{unreadCount > 0 && <span className="absolute -end-0.5 -top-0.5 min-w-4 rounded-full bg-red-600 px-1 text-center text-[10px] font-semibold leading-4 text-white ring-2 ring-white" aria-label={isArabic ? `${unreadCount} غير مقروء` : `${unreadCount} unread`}>{unreadCount > 99 ? "99+" : unreadCount}</span>}<span className="hidden xl:inline ms-1 text-sm font-medium">{isArabic ? "الإشعارات" : "Notifications"}</span>
    </button>
