...
export function BillingSummaryWidget(_props: WidgetComponentProps) {
  ...
  return <div className="min-w-0 space-y-3" dir={direction}>
    <div className="grid min-w-0 grid-cols-3 gap-2 sm:gap-3">
      <div className="min-w-0 overflow-hidden rounded-lg bg-green-50 p-2 text-center sm:p-3"><p className="break-words text-lg font-bold leading-tight text-green-700 sm:text-xl">{currency ? formatCurrency(summary.totalRevenue, currency, locale) : "—"}</p><p className="break-words text-xs text-green-600">{w.billing.collectedRevenue}</p></div>
      <div className="min-w-0 overflow-hidden rounded-lg bg-blue-50 p-2 text-center sm:p-3"><p className="break-words text-lg font-bold leading-tight text-blue-700 sm:text-xl">{summary.paidInvoicesCount.toLocaleString("en-US", { numberingSystem: "latn" })}</p><p className="break-words text-xs text-blue-600">{w.billing.paidInvoices}</p></div>
      <div className="min-w-0 overflow-hidden rounded-lg bg-yellow-50 p-2 text-center sm:p-3"><p className="break-words text-lg font-bold leading-tight text-yellow-700 sm:text-xl">{currency ? formatCurrency(summary.outstandingAmount, currency, locale) : "—"}</p><p className="break-words text-xs text-yellow-600">{w.billing.outstanding} ({summary.outstandingCount.toLocaleString("en-US", { numberingSystem: "latn" })})</p></div>
    </div>
