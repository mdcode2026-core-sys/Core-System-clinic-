...
export function QuickActionsHeaderControl({ isArabic, onSignOut }: QuickActionsHeaderControlProps) {
  ...
  return (
    <div className="relative">
      ...
      <GlobalOverlayPortal open={open} anchorRef={triggerRef} onClose={closeMenu} id="global-quick-actions-menu" ariaLabel={isArabic ? "الإجراءات السريعة" : "Quick actions"} dir={isArabic ? "rtl" : "ltr"} role="dialog" width={320} zIndex={90}>
        <div data-testid="quick-actions-menu" className="min-w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
          ...
          <div className="rounded-lg px-2 py-2 focus-within:bg-slate-50"><LanguageSwitcher /></div>
          <button type="button" onClick={() => void handleSignOut()} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm text-slate-700 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
