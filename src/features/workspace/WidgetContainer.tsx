...
import { useState, Suspense, Component as ReactComponent, type DragEvent, type ReactNode } from "react";
...
import { AlertCircle, ChevronDown, Loader2, Pin } from "lucide-react";
...
function WidgetErrorBoundary({ onRetry, labels, direction }: { onRetry: () => void; labels: { error: string; retry: string }; direction: "rtl" | "ltr" }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-red-200 bg-red-50 p-6 text-red-700" dir={direction}>
      <AlertCircle className="h-8 w-8" />
      <p className="text-sm font-medium">{labels.error}</p>
      <button type="button" onClick={onRetry} className="rounded-md bg-red-100 px-3 py-1.5 text-xs font-medium hover:bg-red-200">{labels.retry}</button>
    </div>
  );
}
...
export function WidgetContainer({
  ...
  return (
    <div
      className={cn(
        "relative flex min-w-0 flex-col rounded-xl border bg-white shadow-sm transition-shadow",
        isPinned && "ring-2 ring-blue-100 shadow-md",
        !isCollapsed && "hover:shadow-md",
        isDropTarget && "border-blue-400 ring-2 ring-blue-100",
      )}
      dir={direction}
    ...
    >
      <div className="flex items-center justify-between border-b px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          {isPinned && <Pin className="h-4 w-4 shrink-0" />}
      ...
      {isCollapsed ? (
        <div className="flex items-center justify-center py-4 text-gray-400">
          ...
          <span className="me-1 text-xs">{workspace.collapsed}</span>
        </div>
      ) : error ? (
        <WidgetErrorBoundary onRetry={handleRetry} labels={{ error: workspace.widgetError, retry: workspace.retry }} direction={direction} />
      ) : (
        <div className="min-w-0 p-4">
          <WidgetRenderBoundary key={retryKey} onError={(err) => setError(err)}>
            <Suspense fallback={<WidgetLoading />}>
              <WidgetContent definition={definition} context={context} />
            </Suspense>
