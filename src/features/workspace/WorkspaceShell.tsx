  ...
  return (
    <div className="min-h-screen w-full bg-gray-50">
      ...
      <aside
        ...
        className={cn(
          "fixed inset-y-0 z-50 w-64 bg-white shadow-lg transition-transform duration-200 ease-out",
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b px-6 py-4">
            ...
            <button type="button" onClick={closeSidebar} className="rounded-md p-1.5 hover:bg-gray-100" aria-label={messages.shell.closeMenu} title={messages.shell.closeMenu}>
              <X className="h-5 w-5" />
            </button>
          </div>
      ...
      <div className="flex min-h-screen min-w-0 flex-col">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-4 py-3 lg:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <button type="button" onClick={openSidebar} className="rounded-md p-1.5 hover:bg-gray-100" aria-label={messages.shell.openMenu} title={messages.shell.openMenu}>
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-4">
            <LanguageSwitcher />
            <span className="hidden max-w-[240px] truncate text-sm text-gray-600 sm:inline">{user?.email}</span>
            <button type="button" onClick={handleSignOut} className="flex items-center gap-1.5 rounded-md bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-200">
              <LogOut className="h-4 w-4" /><span className="hidden sm:inline">{messages.shell.signOut}</span>
            </button>
          </div>
        </header>
