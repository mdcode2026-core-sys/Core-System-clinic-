...
export function WorkspaceShell({ children, user }: WorkspaceShellProps) {
  ...
  const sidebarTransform = sidebarOpen ? "translateX(0)" : locale === "ar" ? "translateX(100%)" : "translateX(-100%)";
  return <div className="flex h-screen w-full bg-gray-50">
    {sidebarOpen && <div className="fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={() => setSidebarOpen(false)} />}
    <aside style={{ transform: sidebarTransform }} className={cn("fixed inset-y-0 start-0 z-50 w-64 transform bg-white shadow-lg transition-transform duration-200 ease-in-out lg:static lg:translate-x-0")}> <div className="flex h-full flex-col">
