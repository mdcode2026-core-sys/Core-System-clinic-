...
export function WorkspaceShell({ children, user }: WorkspaceShellProps) {
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({ "/financial-resources": true });
  ...
  const renderItem = (item: NavItem, nested = false): React.ReactNode => {
    ...
    const toggle = () => setOpenGroups((v) => ({ ...v, [item.href]: !open }));
    const closeOnMobile = () => setMobileSidebarOpen(false);

    return <div key={`${item.href}-${nested ? "nested" : "root"}`}>
      <div className={cn("flex items-center rounded-lg text-sm font-medium transition-colors", nested && (isArabic ? "mr-4" : "ml-4"), active ? "bg-blue-50 text-blue-700" : "text-gray-700 hover:bg-gray-100 hover:text-gray-900")}>
        {expandable ? <button type="button" onClick={toggle} className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2 text-start" aria-expanded={open}>
          <Icon className="h-4 w-4 shrink-0" /><span className="truncate">{getLabel(item)}</span>
        </button> : <Link href={item.href} prefetch onClick={closeOnMobile} className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2">
          <Icon className="h-4 w-4 shrink-0" /><span className="truncate">{getLabel(item)}</span>
        </Link>}
        {expandable && <button type="button" onClick={toggle} className="p-2" aria-label={open ? "Collapse" : "Expand"}><ChevronDown className={cn("h-4 w-4 shrink-0 transition-transform", open && "rotate-180")} /></button>}
      </div>
  ...
  return <div className="flex min-h-screen w-full bg-gray-50">
    ...
    <aside className={cn("fixed inset-y-0 z-50 w-72 bg-white shadow-lg transition-transform duration-200 ease-in-out lg:translate-x-0", isArabic ? "right-0" : "left-0", mobileSidebarOpen ? "translate-x-0" : isArabic ? "translate-x-full" : "-translate-x-full")}>
      <div className="flex h-full flex-col">
        <div className="flex items-center justify-between border-b px-6 py-4"><Link href="/" className="text-xl font-bold text-blue-600" onClick={() => setMobileSidebarOpen(false)}>ClinicSaaS™</Link><button className="rounded p-2 hover:bg-gray-100" onClick={() => setMobileSidebarOpen(false)} aria-label="Close sidebar"><X className="h-5 w-5" /></button></div>
