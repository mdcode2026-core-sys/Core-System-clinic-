...
export function CommunicationsHeaderControl({ isArabic }: CommunicationsHeaderControlProps) {
  ...
  const refresh = useCallback(async () => { const feed = await getCommunicationsHeaderFeed(); setUnread(feed.unread); setRequests(feed.requests); }, []);
  useEffect(() => { const timer = window.setTimeout(() => void refresh(), 0); const interval = window.setInterval(() => void refresh(), 15000); const onFocus = () => void refresh(); window.addEventListener("focus", onFocus); return () => { window.clearTimeout(timer); window.clearInterval(interval); window.removeEventListener("focus", onFocus); }; }, [refresh]);
  ...
  const readAndOpen = async (conversationId: string) => { await markConversationRead(conversationId); setOpen(false); await refresh(); };
  return (
    <div className="relative shrink-0">
      ...
      {open ? <div role="dialog" aria-label={isArabic ? "الاتصالات" : "Communications"} className="fixed inset-x-2 top-[4.25rem] z-[80] w-auto overflow-hidden rounded-xl border border-[var(--cs-slate-200)] bg-white shadow-[var(--cs-shadow-md)] sm:inset-x-auto sm:end-2 sm:w-[min(24rem,calc(100vw-1rem))] md:absolute md:end-0 md:top-11" data-testid="communications-header-panel">
