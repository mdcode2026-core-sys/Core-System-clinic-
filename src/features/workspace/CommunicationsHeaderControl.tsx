...
export function CommunicationsHeaderControl({ isArabic }: CommunicationsHeaderControlProps) {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={openUtility} className="relative inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-600 transition-colors hover:bg-white hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1" aria-label={isArabic ? "تنبيهات وملاحظات الاتصالات" : "Communications alerts and notes"} title={isArabic ? "تنبيهات وملاحظات الاتصالات" : "Communications alerts and notes"} aria-expanded={open} aria-haspopup="dialog" data-testid="global-header-communications">
        <Mail className="h-[19px] w-[19px]" aria-hidden="true" />
        {utilityCount > 0 ? <span className="absolute -end-0.5 -top-1 min-w-5 rounded-full bg-red-600 px-1.5 text-center text-[10px] font-bold leading-5 text-white ring-2 ring-slate-50" aria-label={isArabic ? `${utilityCount} جديد` : `${utilityCount} new`}>{utilityCount > 99 ? "99+" : utilityCount}</span> : null}
      </button>

      {open ? <div role="dialog" aria-label={isArabic ? "تنبيهات وملاحظات الاتصالات" : "Communications alerts and notes"} className="absolute end-0 top-11 z-[70] w-[min(22rem,calc(100vw-1rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><strong className="text-sm text-slate-900">{isArabic ? "الاتصالات" : "Communications"}</strong><Link href="/communications" onClick={() => setOpen(false)} className="text-xs font-medium text-blue-600 hover:text-blue-700">{isArabic ? "فتح المركز" : "Open center"}</Link></div>
