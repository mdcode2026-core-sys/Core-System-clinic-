...
export function CommunicationsHeaderControl({ isArabic }: CommunicationsHeaderControlProps) {
  ...
  return (
    <div className="relative">
      ...
      {open ? <div role="dialog" aria-label={isArabic ? "تنبيهات وملاحظات الاتصالات" : "Communications alerts and notes"} className="fixed inset-x-2 top-[4.25rem] z-[80] w-auto overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl md:absolute md:inset-x-auto md:end-0 md:top-11 md:w-[min(22rem,calc(100vw-1rem))">
        <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3"><strong className="text-sm text-slate-900">{isArabic ? "الاتصالات" : "Communications"}</strong><Link href="/communications" onClick={() => setOpen(false)} className="shrink-0 text-xs font-medium text-blue-600 hover:text-blue-700">{isArabic ? "فتح المركز" : "Open center"}</Link></div>
