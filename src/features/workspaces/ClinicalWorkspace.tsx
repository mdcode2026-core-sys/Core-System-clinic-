...
import { CheckCircle2, Clock, DoorOpen, GripVertical, RefreshCw, Stethoscope, UserRound } from "lucide-react";
...
export function ClinicalWorkspace({ initialQueue = [] }: { initialQueue?: EnrichedSession[] }) {
  ...
  const handleClinicalDrop = async () => {
    if (!draggedId || current) return;
    const session = sessions.find((item) => item.id === draggedId);
    setDraggedId(null);
    if (!session || session.session_status !== "waiting" || session.lock_holder_id) return;
  ...
  return <div className="mx-auto max-w-6xl space-y-6" dir={direction}>
    ...
    </div> : <Card onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); void handleClinicalDrop(); }} className="border-dashed border-primary/40"><CardContent className="p-8 text-center"><DoorOpen className="mx-auto mb-2 h-10 w-10 text-muted-foreground" /><p className="text-muted-foreground">{c.noActiveVisit}</p>{waiting.length > 0 && <p className="mt-2 text-xs text-muted-foreground">{locale === "ar" ? "اسحب المريض إلى هذه المساحة لبدء المعاينة." : "Drag a waiting patient into this area to start clinical work."}</p>}{waiting.length > 0 && <Button className="mt-4" size="lg" onClick={() => void startNext(waiting[0])}>{c.takeNext}</Button>}</CardContent></Card>}
    <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5" /> {c.waitingPatients} ({waiting.length})</CardTitle></CardHeader><CardContent className="space-y-2">{waiting.length === 0 ? <p className="py-4 text-center text-sm text-muted-foreground">{c.noWaiting}</p> : waiting.map((session) => <div key={session.id} draggable={!current && !session.lock_holder_id} onDragStart={() => setDraggedId(session.id)} onDragEnd={() => setDraggedId(null)} className="flex items-center justify-between gap-3 rounded-lg border p-3 cursor-grab active:cursor-grabbing"><div className="min-w-0"><p className="font-medium truncate">{session.patient_name}</p><p className="text-xs text-muted-foreground">{session.doctor_name || c.providerUnspecified} · {c.wait} {session.wait_time_minutes ?? 0} {c.minutes}</p></div><div className="flex items-center gap-2"><GripVertical className="h-4 w-4 text-muted-foreground" aria-hidden="true" /><Button size="sm" disabled={!!current || !!session.lock_holder_id} onClick={() => void startNext(session)}>{c.take}</Button></div></div>)}</CardContent></Card><Card><CardHeader><CardTitle>{c.returnedToOperation} ({returned.length})</CardTitle></CardHeader><CardContent>{returned.length === 0 ? <p className="py-4 text-center text-sm text-muted-foreground">{c.noPendingClose}</p> : returned.map((session) => <div key={session.id} className="rounded-lg border p-3 mb-2"><p className="font-medium">{session.patient_name}</p><p className="text-xs text-muted-foreground">{c.pendingOperationClose}</p></div>)}</CardContent></Card></div>
  </div>;
}
