...
export function ClinicalWorkspace({ initialQueue = [] }: { initialQueue?: EnrichedSession[] }) {
  ...
  const finish = async () => { if (!current) return; setSaving(true); setError(null); try { await finishClinicalVisit(current.id, { examination, findings, decision }); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : c.finishFailed); } finally { setSaving(false); } };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-full bg-primary/10 p-3"><Stethoscope className="h-6 w-6 text-primary" /></div><div><h1 className="text-3xl font-bold">{c.title}</h1><p className="text-muted-foreground mt-1">{c.description}</p></div></div><div className="flex gap-2"><Button variant="outline" onClick={() => void refresh()} disabled={loading}><RefreshCw className={`h-4 w-4 me-2 ${loading ? "animate-spin" : ""}`} /> {c.refresh}</Button>{current && <Button variant="outline" onClick={() => router.push(`/treatment-plans?patientId=${encodeURIComponent(current.patient_id)}&visitId=${encodeURIComponent(current.id)}`)}>{c.treatmentPlan}</Button>}</div></div>
