...
export function ClinicalWorkspace({ initialQueue = [] }: { initialQueue?: EnrichedSession[] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "تعذر تحميل مساحة العمل السريرية"); }
    finally { setLoading(false); }
  }, [user, procedures.length]);

  useEffect(() => { void refresh(); }, [refresh]);
  const run = async (fn: () => Promise<unknown>) => { setError(null); try { await fn(); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : "تعذر تنفيذ الإجراء"); } };
  const startNext = async (session: EnrichedSession) => { await run(async () => { await transitionToClinical(session.id); }); };
  const save = async () => { if (!current) return; setSaving(true); setError(null); try { await saveClinicalVisit(current.id, { examination, findings, decision }); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : "تعذر حفظ الزيارة"); } finally { setSaving(false); } };
  const finish = async () => { if (!current) return; setSaving(true); setError(null); try { await finishClinicalVisit(current.id, { examination, findings, decision }); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : "تعذر إنهاء الزيارة"); } finally { setSaving(false); } };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6" dir="rtl">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-full bg-primary/10 p-3"><Stethoscope className="h-6 w-6 text-primary" /></div><div><h1 className="text-3xl font-bold">مساحة العمل السريرية</h1><p className="text-muted-foreground mt-1">الزيارة السريرية والفحص والنتائج والقرار الطبي</p></div></div><div className="flex gap-2"><Button variant="outline" onClick={() => void refresh()} disabled={loading}><RefreshCw className={`h-4 w-4 ml-2 ${loading ? "animate-spin" : ""}`} /> تحديث</Button>{current && <Button variant="outline" onClick={() => router.push(`/treatment-plans?patientId=${encodeURIComponent(current.patient_id)}&visitId=${encodeURIComponent(current.id)}`)}>خطة العلاج</Button>}</div></div>
      {error && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}
      {current ? (
        <div className="space-y-4">
          <Card className="border-primary/30"><CardHeader><CardTitle className="flex items-center gap-2"><UserRound className="h-5 w-5" /> سياق الزيارة</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-4 text-sm"><div><span className="text-muted-foreground">المريض</span><p className="font-semibold">{current.patient_name || "مريض"}</p><p className="text-xs text-muted-foreground">{current.patient_file_number || "بدون رقم ملف"}</p></div><div><span className="text-muted-foreground">المقدم</span><p className="font-semibold">{current.doctor_name || "غير محدد"}</p></div><div><span className="text-muted-foreground">الغرفة</span><p className="font-semibold">{current.room_name || "غير محددة"}</p></div><div><span className="text-muted-foreground">الموعد</span><p className="font-semibold">{current.agenda_event_id ? "مرتبط بموعد" : "زيارة بدون موعد"}</p></div></CardContent></Card>
