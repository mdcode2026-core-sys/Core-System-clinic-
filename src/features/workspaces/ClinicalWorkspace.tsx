...
export function ClinicalWorkspace({ initialQueue = [] }: { initialQueue?: EnrichedSession[] }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6" dir={direction}>
    ...
    </div> : <Card><CardContent className="p-8 text-center"><DoorOpen className="mx-auto mb-2 h-10 w-10 text-muted-foreground" /><p className="text-muted-foreground">{c.noActiveVisit}</p>{waiting.length > 0 && <Button className="mt-4" size="lg" onClick={() => void startNext(waiting[0])}>{c.takeNext}</Button>}</CardContent></Card>}
