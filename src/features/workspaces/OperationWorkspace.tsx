...
export function OperationWorkspace({ initialQueue = [] }: { initialQueue?: EnrichedSession[] }) {
  ...
  const dropTarget = async (target: SessionStatus) => {
    ...
    if (!allowed) { setError(o.invalidTransition); return; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
        ...
        <Button variant="outline" onClick={() => void refresh()} disabled={loading}><RefreshCw className={`h-4 w-4 me-2 ${loading ? "animate-spin" : ""}`} /> {o.refresh}</Button>
      </div>
          ...
          return <section key={lane.status} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); void dropTarget(lane.status); }} className="min-h-[420px] rounded-xl border bg-muted/20 p-3">
            ...
            <div className="space-y-3">
              {items.map((session) => <Card key={session.id} draggable={session.session_status !== "completed"} onDragStart={() => setDraggedId(session.id)} onDragEnd={() => setDraggedId(null)} className="cursor-grab active:cursor-grabbing">
                <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-base"><GripVertical className="h-4 w-4 text-muted-foreground" /><span className="truncate">{session.patient_name || o.patientFallback}</span></CardTitle></CardHeader>
                <CardContent className="space-y-3 text-sm">
                  ...
                  {session.session_status === "waiting" && <div className="flex flex-wrap gap-2">
                    <Button size="sm" onClick={() => void run(() => moveFromOperation(session.id, "in_consultation"))}><UserCheck className="h-4 w-4 me-1" /> {o.directToProvider}</Button>
                    <Button size="sm" variant="outline" onClick={() => void run(() => registerPatientArrival({ sessionId: session.id, patient_id: session.patient_id }))}>{o.registerArrival}</Button>
                    <Button size="sm" variant="ghost" onClick={() => void run(() => markNoShowFromReception(session.id))}>{o.noShow}</Button>
                    <Button size="sm" variant="ghost" className="text-destructive" onClick={() => void run(() => cancelFromReception(session.id))}>{o.cancel}</Button>
                  </div>}
