...
interface Props { initialData: InvoiceListItem[]; initialError: string | null; permissions?: Permission[]; }

export function InvoiceList({ initialData, initialError, permissions: serverPermissions }: Props) {
  ...
  if (invoices.length === 0) return <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5" />{invoice.title}</CardTitle></CardHeader><CardContent><div className="text-center py-8 text-muted-foreground">{invoice.noInvoices}</div>{canCreate && <div className="flex justify-center mt-4"><Button onClick={() => router.push("/invoices/new")}><Plus className="w-4 h-4 ml-2" />{invoice.createNew}</Button></div>}</CardContent></Card>;

  return <Card dir={locale === "ar" ? "rtl" : "ltr"}><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5" />{invoice.title}</CardTitle>{canCreate && <Button onClick={() => router.push("/invoices/new")}><Plus className="w-4 h-4 ml-2" />{invoice.createNew}</Button>}</CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b">
