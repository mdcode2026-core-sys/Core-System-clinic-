...
interface Props {
  ...
  initialProcedures: ProcedureOption[];
  initialSessions: SessionOption[];
  initialPatientsError: string | null;
  initialProceduresError: string | null;
...
export function InvoiceForm({
  ...
  initialProcedures,
  initialSessions,
  initialPatientsError,
  initialProceduresError,
  ...
  const [sessions] = useState<SessionOption[]>(initialSessions);
  ...
  function addItem() {
    const newItem: InvoiceFormItem = {
      tempId: `${idPrefix}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      procedure_id: null,
      description: "",
      quantity: 1,
                ...
                <SelectContent>
                  ...
                  {sessions.map((s) => (
                    <SelectItem key={s.id} value={s.id}>
                      ...
                      {new Date(s.session_started_at).toLocaleDateString("ar-JO")}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateItem(item.tempId, {
                          unit_price_subunits: Math.round(
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateItem(item.tempId, {
                          discount_amount_subunits: Math.round(
                            parseFloat(e.target.value) * 100
                          ),
                        })
                      }
