...
export function ReportViewer({ reportLabel, result }: ReportViewerProps) {
  ...
  const format = (value: unknown): string => {
    ...
    if (typeof value === "number") return value.toLocaleString(dateLocale);
  ...
  return <div className="overflow-hidden rounded-lg border bg-card text-card-foreground shadow-sm print:border-0 print:shadow-none" dir={locale === "ar" ? "rtl" : "ltr"}>
    <div className="mb-4 hidden print:block"><h1 className="text-xl font-bold">ClinicSaaS™ — {reportLabel}</h1><p className="text-sm text-muted-foreground">{new Date().toLocaleDateString(dateLocale)}</p></div>
    <div className="border-b bg-muted/50 p-4 print:hidden"><h3 className="font-semibold">{reportLabel}</h3><p className="text-xs text-muted-foreground">{rows.length} {rows.length === 1 ? t.record : t.records}</p></div>
