...
export function ReportViewer({ reportLabel, result }: ReportViewerProps) {
  const { locale, reportViewer: t } = useI18n(); const { columns, rows, summary } = result;
  const dateLocale = locale === "ar" ? "ar" : "en-US"; const numeric = { numberingSystem: "latn" as const };
  const format = (value: unknown): string => { if (value === null || value === undefined) return t.unknown; if (typeof value === "boolean") return value ? t.yes : t.no; if (typeof value === "number") return value.toLocaleString(dateLocale, numeric); if (value instanceof Date) return value.toLocaleDateString(dateLocale, numeric); return String(value); };
  return <div className="overflow-hidden rounded-lg border bg-card text-card-foreground shadow-sm print:border-0 print:shadow-none" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <div className="border-b bg-muted/50 p-4 print:hidden"><h3 className="font-semibold">{reportLabel}</h3><p className="text-xs text-muted-foreground">{rows.length.toLocaleString(dateLocale, numeric)} {rows.length === 1 ? t.record : t.records}</p></div>
