...
import { ExternalLink, FileText, Image as ImageIcon, RefreshCw, Upload, X } from "lucide-react";
...
export function MedicalFilesPanel({ patientId, visitId }: { patientId?: string; visitId?: string }) {
  ...
  return <section className="rounded-xl border bg-card p-4 shadow-sm" aria-label="Medical Files">
    ...
    {selected && <div className="mb-4 rounded-xl border bg-muted/20 p-2"><div className="mb-2 flex items-center justify-between"><span className="truncate text-sm font-medium">{selected.original_filename}</span><button type="button" className="rounded p-1 hover:bg-muted" onClick={() => { setSelected(null); setPreviewUrl(null); }}><X className="h-4 w-4" /></button></div>{selected.file_kind === "dicom" ? <DicomViewer fileId={selected.id} /> : previewUrl ? <div className="max-h-[520px] overflow-auto rounded bg-black p-2 text-center"><img src={previewUrl} alt={selected.original_filename} className="mx-auto max-h-[500px] object-contain" /></div> : null}</div>}
