...
import { Enums, RenderingEngine, StackViewport, init as coreInit } from "@cornerstonejs/core";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await viewport.setStack([`wadouri:${url}`], 0);
        ...
        for (const row of saved) {
          const item = row.payload as any;
          if (item?.metadata?.toolName && item?.data) annotation.state.addAnnotation(item, item.metadata.frameOfReferenceUID || item.metadata.FrameOfReferenceUID);
        }
        setLoading(false);
      } catch {
