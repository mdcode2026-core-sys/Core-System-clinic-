...
export async function createEmployeeAdvance(input: { employee_id: string; advance_type: "short_term" | "installment"; amount_subunits: number; currency: string; disbursed_on: string; deduction_payroll_period_id?: string; installment_amount_subunits?: number; installment_frequency?: "monthly"; installment_start_date?: string; reason?: string; }) {
  ...
  const advanceType = input.advance_type;
  const deductionPeriod = cleanOptional(input.deduction_payroll_period_id);
  const installmentStart = cleanOptional(input.installment_start_date);
  const installmentAmount = input.installment_amount_subunits && input.installment_amount_subunits > 0 ? input.installment_amount_subunits : undefined;
  ...
  const { data, error } = await ctx.supabase.rpc("create_workforce_employee_advance", {
    p_tenant_id: ctx.tenantId, p_employee_id: input.employee_id, p_advance_type: advanceType,
    p_amount_subunits: Math.max(0, input.amount_subunits), p_currency: input.currency, p_disbursed_on: input.disbursed_on,
    p_deduction_payroll_period_id: deductionPeriod ?? null, p_installment_amount_subunits: installmentAmount ?? null,
    p_installment_frequency: advanceType === "installment" ? "monthly" : null, p_installment_start_date: installmentStart ?? null,
    p_reason: cleanOptional(input.reason) ?? null, p_created_by: ctx.clinicUser.id,
  });
  if (error || !data) return { success: false, error: error?.message || "Unable to create advance" } as const;
...
export async function createPayrollDeduction(input: { employee_id: string; payroll_period_id: string; deduction_type: string; amount_subunits: number; currency: string; source_type: string; reason?: string; }) {
  ...
  if (!ctx) return { success: false, error: "Unauthorized" } as const;
  if (!(await hasEffectivePermission(ctx.user.id, "workforce:payroll"))) return { success: false, error: "Permission denied" } as const;
  const { data, error } = await ctx.supabase.rpc("create_workforce_payroll_deduction", {
    p_tenant_id: ctx.tenantId, p_employee_id: input.employee_id, p_payroll_period_id: input.payroll_period_id,
    p_deduction_type: input.deduction_type, p_amount_subunits: Math.max(0, input.amount_subunits), p_currency: input.currency,
    p_source_type: input.source_type, p_source_id: null, p_reason: cleanOptional(input.reason) ?? null, p_created_by: ctx.clinicUser.id,
  });
  if (error || !data) return { success: false, error: error?.message || "Unable to create deduction" } as const;
  if (data.success === false) return data as { success: false; error: string };
  revalidatePath("/workforce"); revalidatePath("/workforce/payroll");
  return { success: true, data } as const;
}
