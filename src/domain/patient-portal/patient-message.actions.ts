...
async function patientContext() {
  ...
  if (!(await hasEntitlement(relationship.tenant_id, "patient_experience.advanced"))) throw new Error("Advanced Patient Experience is not enabled");
  return { supabase, user, relationship };
}

export async function listPatientMessages() {
  const { supabase, relationship } = await patientContext();
...
export async function sendPatientMessage(body: string) {
  const text = body.trim();
  if (!text || text.length > 10000) throw new Error("Message must be between 1 and 10000 characters");
  const { supabase, user, relationship } = await patientContext();
