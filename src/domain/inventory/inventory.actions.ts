...
const INVALID_TYPE = "INVENTORY_INVALID_TRANSACTION_TYPE";
const QUANTITY_INVALID = "INVENTORY_QUANTITY_INVALID";
const NEGATIVE_STOCK = "INVENTORY_NEGATIVE_STOCK";
const DATABASE_ERROR = "INVENTORY_DATABASE_ERROR";
...
export async function createInventoryItem(formData: FormData) {
  ...
  if (!(await allowed(ctx.user.id, "inventory:create"))) return { error: "INVENTORY_PERMISSION_DENIED" };

  const name = String(formData.get("name") || "").trim();
  if (!name) return { error: NAME_REQUIRED };

  const requestedOpeningStock = Number(formData.get("current_stock") || 0);
  if (!Number.isInteger(requestedOpeningStock) || requestedOpeningStock < 0) {
  ...
  if (requestedOpeningStock > 0) {
    const { error: openingError } = await ctx.supabase.rpc("adjust_inventory_stock", {
      p_item_id: data.id,
      p_tenant_id: ctx.tenantId,
      p_delta: requestedOpeningStock,
      p_movement_type: "adjustment",
      p_source_type: "manual_adjustment",
      p_source_id: null,
      p_procedure_id: null,
      p_session_id: null,
      p_treatment_plan_item_id: null,
      p_actor_id: ctx.clinicUser.id,
      p_reason: "Opening balance",
    });
    if (openingError) {
      console.error("[createInventoryItem] opening balance error:", openingError.message);
      return { error: DATABASE_ERROR };
    }
  }
...
export async function updateInventoryItem(formData: FormData) {
  const ctx = await getAuthContext();
  if (!ctx) return { error: TENANT_MISSING };
  if (!(await allowed(ctx.user.id, "inventory:update"))) return { error: "INVENTORY_PERMISSION_DENIED" };

  const id = String(formData.get("id") || "");
  if (!id) return { error: INCOMPLETE };
  const name = String(formData.get("name") || "").trim();
  if (!name) return { error: NAME_REQUIRED };
...
export async function adjustStock(formData: FormData) {
  const ctx = await getAuthContext();
  if (!ctx) return { error: TENANT_MISSING };
  if (!(await allowed(ctx.user.id, "inventory:adjust"))) return { error: "INVENTORY_PERMISSION_DENIED" };

  const itemId = String(formData.get("item_id") || "");
  const transactionType = String(formData.get("transaction_type") || "") as InventoryTransactionType;
  const reason = String(formData.get("reason") || "").trim();
  if (!itemId || !transactionType || !reason) return { error: INCOMPLETE };
  if (!(transactionType in TRANSACTION_EFFECT)) return { error: INVALID_TYPE };

  const quantity = Math.abs(Number(formData.get("quantity") || 0));
  if (!Number.isInteger(quantity) || quantity <= 0) return { error: QUANTITY_INVALID };

  const quantityDelta = TRANSACTION_EFFECT[transactionType] * quantity;
  const movementType =
    transactionType === "purchase" ? "purchase_receipt" :
    transactionType === "purchase_return" ? "purchase_return" :
    transactionType === "doctor_request" ? "procedure_consumption" :
    transactionType === "unused_return" ? "unused_return" :
    "adjustment";
  const sourceType =
    transactionType === "purchase" ? "purchase_receipt" :
    transactionType === "purchase_return" ? "purchase_return" :
    transactionType === "doctor_request" ? "procedure_session" :
    transactionType === "unused_return" ? "unused_return" :
    "manual_adjustment";

  const { data: updated, error: updateError } = await ctx.supabase.rpc("adjust_inventory_stock", {
    p_item_id: itemId,
    p_tenant_id: ctx.tenantId,
  ...
  if (updateError) {
    ...
    console.error("[adjustStock] update error:", updateError.message);
    return { error: DATABASE_ERROR };
  }
...
export async function softDeleteInventoryItem(formData: FormData) {
  ...
  const { error } = await ctx.supabase
    ...
    .eq("tenant_id", ctx.tenantId);
  if (error) {
    console.error("[softDeleteInventoryItem] error:", error.message);
    return { error: DATABASE_ERROR };
  }
  revalidatePath("/inventory");
  return { success: true };
