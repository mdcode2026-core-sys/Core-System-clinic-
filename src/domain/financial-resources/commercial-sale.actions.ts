...
export async function executeCommercialSale(input: CommercialSaleInput) {
  ...
  if (!clinicUser?.is_active || clinicUser.deleted_at) return { success: false, error: "Clinic user not found" } as const;
  ...
  const data = result.data as { success: boolean; error?: string; gross_subunits?: number; discount_subunits?: number; net_subunits?: number; patient_package_id?: string; financial_plan_id?: string; invoice_id?: string; service_id?: string; package_id?: string; offer_id?: string };
  if (data.success !== true) return data;
  revalidatePath(`/patients/${input.patientId}`);
  revalidatePath("/invoices");
  revalidatePath("/financial-resources");
  return data;
}
