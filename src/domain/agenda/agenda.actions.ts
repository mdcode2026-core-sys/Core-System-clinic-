...
async function resolveBufferEnd(supabase: Awaited<ReturnType<typeof createClient>>, procedureId: string | null, scheduledEnd: string) {
  ...
  const end = new Date(scheduledEnd); end.setMinutes(end.getMinutes() + Math.min(bufferMinutes, 120)); return end.toISOString();
}
async function validateResource(supabase: Awaited<ReturnType<typeof createClient>>, tenantId: string, resourceId: string | null) {
  ...
  const { data } = await supabase.from("clinic_resources").select("id").eq("id", resourceId).eq("tenant_id", tenantId).eq("status", "active").is("deleted_at", null).maybeSingle();
  return !!data;
}
...
export async function createAgendaEvent(formData: FormData) {
  ...
  const bufferEnd = await resolveBufferEnd(ctx.supabase, procedureId, scheduledEnd); const conflictResult = await checkConflicts(ctx.supabase, { tenantId: ctx.tenantId, doctorId, roomId, resourceId, patientId, scheduledStart, scheduledEnd, bufferEnd }); if (conflictResult.hasConflict) return { error: conflictResult.message };
...
export async function updateAgendaEvent(formData: FormData) {
  ...
  const timeValidation = isValidTimeRange(scheduledStart, scheduledEnd); if (!timeValidation.valid) return { error: timeValidation.message }; if (!(await validateResource(ctx.supabase, ctx.tenantId, resourceId))) return { error: "AGENDA_RESOURCE_UNAVAILABLE" };
  const bufferEnd = await resolveBufferEnd(ctx.supabase, procedureId, scheduledEnd); const conflictResult = await checkConflicts(ctx.supabase, { tenantId: ctx.tenantId, doctorId, roomId, resourceId, patientId, scheduledStart, scheduledEnd, bufferEnd, excludeEventId: eventId }); if (conflictResult.hasConflict) return { error: conflictResult.message };
...
export async function updateAgendaEventStatus(formData: FormData) {
  const ctx = await resolveContext(); if (!ctx) return { error: TENANT_MISSING }; if (!can(ctx.permissions, "agenda:update")) return { error: PERMISSION_DENIED }; const eventId = String(formData.get("id") || ""); const newStatus = String(formData.get("status") || "") as AgendaEventStatusValue; const currentStatus = String(formData.get("current_status") || "") as AgendaEventStatusValue; const allowedTransitions = ValidStateTransitions[currentStatus]; if (!allowedTransitions || !allowedTransitions.includes(newStatus)) return { error: `AGENDA_INVALID_STATUS_TRANSITION|${currentStatus}|${newStatus}` };
  const { data, error } = await ctx.supabase.from("master_agenda_events").update({ status: newStatus, updated_at: new Date().toISOString() }).eq("id", eventId).eq("tenant_id", ctx.tenantId).select().single(); if (error) { console.error("[updateAgendaEventStatus] error:", error.message); return { error: DATABASE_ERROR }; } return { data };
}

export async function cancelAgendaEvent(formData: FormData) {
  const ctx = await resolveContext(); if (!ctx) return { error: TENANT_MISSING }; if (!can(ctx.permissions, "agenda:update")) return { error: PERMISSION_DENIED }; const eventId = String(formData.get("id") || ""); const currentStatus = String(formData.get("current_status") || "") as AgendaEventStatusValue; const cancellableStates: AgendaEventStatusValue[] = ["scheduled", "confirmed", "arrived", "in_session"]; if (!cancellableStates.includes(currentStatus)) return { error: `AGENDA_INVALID_CANCELLATION|${currentStatus}` }; const cancellationReason = String(formData.get("cancellation_reason") || "") || null;
  const { data, error } = await ctx.supabase.from("master_agenda_events").update({ status: AgendaEventStatus.CANCELLED, cancellation_reason: cancellationReason, updated_at: new Date().toISOString() }).eq("id", eventId).eq("tenant_id", ctx.tenantId).select().single(); if (error) { console.error("[cancelAgendaEvent] error:", error.message); return { error: DATABASE_ERROR }; } return { data };
}
