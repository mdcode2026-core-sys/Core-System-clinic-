...
export const ValidStateTransitions: Record<string, AgendaEventStatusValue[]> = {[AgendaEventStatus.SCHEDULED]: [AgendaEventStatus.CONFIRMED, AgendaEventStatus.CANCELLED, AgendaEventStatus.RESCHEDULED],[AgendaEventStatus.CONFIRMED]: [AgendaEventStatus.ARRIVED, AgendaEventStatus.NO_SHOW, AgendaEventStatus.CANCELLED, AgendaEventStatus.RESCHEDULED],[AgendaEventStatus.ARRIVED]: [AgendaEventStatus.IN_SESSION, AgendaEventStatus.NO_SHOW, AgendaEventStatus.CANCELLED],[AgendaEventStatus.IN_SESSION]: [AgendaEventStatus.COMPLETED, AgendaEventStatus.CANCELLED],[AgendaEventStatus.COMPLETED]: [],[AgendaEventStatus.NO_SHOW]: [AgendaEventStatus.RESCHEDULED],[AgendaEventStatus.CANCELLED]: [AgendaEventStatus.SCHEDULED],[AgendaEventStatus.RESCHEDULED]: [AgendaEventStatus.SCHEDULED]};
...
export interface AvailabilityCheckInput { tenantId: string; doctorId: string; roomId: string | null; resourceId?: string | null; procedureId?: string | null; scheduledStart: string; scheduledEnd: string; bufferEnd?: string; }
export interface AvailabilityResult { isAvailable: boolean; reason: string | null; details: { doctorAvailable: boolean; roomAvailable: boolean; resourceAvailable?: boolean; providerEligible: boolean; withinWorkingHours: boolean; notBlocked: boolean; }; }
...
export interface AgendaEventWithRelations extends AgendaEventRow { status: AgendaEventStatusValue; notes: string | null; patient?: { id: string; first_name: string; last_name: string; phone_primary: string; } | null; doctor?: { id: string; full_name: string; full_name_ar: string | null; specialization: string | null; } | null; room?: { id: string; room_name: string; } | null; resource?: { id: string; resource_name: string; resource_type: string; status: string; } | null; procedure?: { id: string; procedure_name: string; standard_duration_minutes: number; buffer_time_minutes: number; } | null; }
