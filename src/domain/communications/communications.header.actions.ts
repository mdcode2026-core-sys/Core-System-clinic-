...
export async function searchCommunicationUsers(query = "") {
  ...
  let request = ctx.supabase
    ...
    .limit(20);
  if (value) {
    request = request.or(`full_name.ilike.%${value}%,email.ilike.%${value}%`);
  }
...
export async function getCommunicationsHeaderFeed() {
  ...
  if (conversationIds.length) {
    const { data: messages } = await ctx.supabase
      ...
      .select("id,conversation_id,body,message_kind,created_at")
      .eq("tenant_id", ctx.tenantId)
      .in("conversation_id", conversationIds)
      .in("message_kind", ["message", "internal_note"])
      .neq("sender_clinic_user_id", ctx.clinicUser.id)
  ...
  const { data: requests } = await ctx.supabase
    ...
    .select("id,title,priority,status,due_at,updated_at")
    .eq("tenant_id", ctx.tenantId)
    .eq("assignee_clinic_user_id", ctx.clinicUser.id)
