...
async function transitionToPendingReception(sessionId: string) {
  const { transitionToPendingReception: transition } = await import("@/domain/queue/workspace.actions");
  await transition(sessionId);
}
