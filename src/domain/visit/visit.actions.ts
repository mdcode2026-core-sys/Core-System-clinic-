...
export async function getClinicalVisit(sessionId: string): Promise<ClinicalVisitRecord | null> {
  ...
  const { data, error } = await supabase
    ...
    .select(`id,tenant_id,patient_id,doctor_id,room_id,agenda_event_id,session_status,session_started_at,session_ended_at,visit_closed_at,examination,findings,decision,clinic_patients(first_name,last_name,phone_primary,file_number),clinic_users!clinic_visit_sessions_doctor_id_fkey(full_name),clinic_rooms(room_name),clinic_visit_procedures(id,procedure_id,quantity,notes,performed_at,clinic_procedures(procedure_name))`)
    .eq("id", sessionId)
  ...
  return {
    ...
    visit_closed_at: row.visit_closed_at,
    doctor_name: row.clinic_users?.full_name ?? null,
    room_name: row.clinic_rooms?.room_name ?? null,
    patient_name: row.clinic_patients ? `${row.clinic_patients.first_name} ${row.clinic_patients.last_name}` : null,
    patient_file_number: row.clinic_patients?.file_number ?? null,
    patient_phone: row.clinic_patients?.phone_primary ?? null,
    examination: typeof row.examination === "string" ? row.examination : row.examination?.text ?? "",
    findings: typeof row.findings === "string" ? row.findings : row.findings?.text ?? "",
    decision: typeof row.decision === "string" ? row.decision : row.decision?.text ?? "",
    procedures: (row.clinic_visit_procedures ?? []).map((p: any) => ({
      id: p.id,
      procedure_id: p.procedure_id,
      procedure_name: p.clinic_procedures?.procedure_name ?? "",
      quantity: p.quantity,
      notes: p.notes ?? null,
...
export async function getClinicalProcedures() {
  ...
  if (error) throw new Error(`Procedure fetch failed: ${error.message}`);
...
export async function saveClinicalVisit(sessionId: string, input: ClinicalVisitData): Promise<void> {
  ...
  const examination = input.examination.trim();
  const findings = input.findings.trim();
  const decision = input.decision.trim();
  if (!examination && !findings && !decision) throw new Error("Clinical documentation cannot be completely empty");

  const { error } = await supabase.from("clinic_visit_sessions").update({
    examination: examination ? { text: examination } : null,
    findings: findings ? { text: findings } : null,
    decision: decision ? { text: decision } : null,
    clinical_notes: findings || null,
    doctor_notes: decision || null,
    updated_at: new Date().toISOString(),
  }).eq("id", sessionId).eq("tenant_id", tenantId).eq("session_status", "in_consultation");
  if (error) throw new Error(`Clinical visit save failed: ${error.message}`);
...
export async function addVisitProcedure(sessionId: string, procedureId: string, quantity = 1, notes?: string): Promise<void> {
  ...
  if (sessionError || !session || session.session_status !== "in_consultation") throw new Error("Visit is not active");

  const { error } = await supabase.from("clinic_visit_procedures").upsert({
    tenant_id: tenantId,
    visit_id: sessionId,
    ...
    created_by: user.id,
    updated_at: new Date().toISOString(),
  }, { onConflict: "visit_id,procedure_id" });
  if (error) throw new Error(`Procedure save failed: ${error.message}`);
  revalidatePath("/(dashboard)/clinical");
}
...
export async function removeVisitProcedure(procedureRowId: string): Promise<void> {
  ...
  if (error) throw new Error(`Procedure removal failed: ${error.message}`);
  revalidatePath("/(dashboard)/clinical");
}

export async function finishClinicalVisit(sessionId: string, input: ClinicalVisitData): Promise<void> {
  await saveClinicalVisit(sessionId, input);
  await transitionToPendingReception(sessionId);
