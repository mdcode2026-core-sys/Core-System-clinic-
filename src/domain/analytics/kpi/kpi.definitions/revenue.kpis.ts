...
async function doctorBreakdown(supabase: any, tenantId: string, dateRange: any): Promise<AnalyticsBreakdownRow[]> {
  ...
  for (const invoice of invoices) {
    ...
    current.value += Number(invoice.total_subunits ?? 0); current.count += 1; groups.set(key, current);
  }
...
async function serviceBreakdown(supabase: any, tenantId: string, dateRange: any): Promise<AnalyticsBreakdownRow[]> {
  ...
  for (const item of items) {
    ...
    const current = groups.get(key) ?? { value: 0, count: 0 };
    current.value += Number(item.line_total_subunits ?? 0); current.count += Number(item.quantity ?? 1); groups.set(key, current);
  }
...
export const revenueAvgInvoiceKpi: KpiDefinition = { id: "revenue.avg_invoice", nameAr: "صافي المحصل", category: "revenue", dateBasis: "payment_date", sourceTables: ["invoice_payments","invoice_refunds"], supportsDateFilter: true, businessDefinition: "Net Collected = payment transactions خلال الفترة ناقص refunds خلال الفترة.", calculator: async (s,t,r)=>revenueCollectedForPeriod(s,t,r)-sumRefunded(s,t,r), formatter:kpiFormatter.currency };
async function revenueCollectedForPeriod(s: any,t: string,r: any){const {data,error}=await s.from("invoice_payments").select("amount_subunits").eq("tenant_id",t).gte("payment_date",r.startAt).lt("payment_date",r.endAtExclusive);if(error)throw error;return(data??[]).reduce((x:number,row:any)=>x+Number(row.amount_subunits??0),0)}
...
export const revenueNetCollectedKpi: KpiDefinition = { id: "revenue.net_collected", nameAr: "صافي المحصل", category: "revenue", dateBasis: "payment_date", sourceTables: ["invoice_payments","invoice_refunds"], supportsDateFilter: true, businessDefinition: "Collected payment transactions minus refunds during the selected period.", calculator: async(s,t,r)=>revenueCollectedForPeriod(s,t,r)-await sumRefunded(s,t,r), formatter:kpiFormatter.currency };
