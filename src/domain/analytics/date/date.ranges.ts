...
function isoDate({ year, month, day }: Parts): string { return `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`; }
function parseDate(value: string): Parts { const [year, month, day] = value.split("-").map(Number); if (![year, month, day].every(Number.isInteger)) throw new Error(`Invalid analytics date: ${value}`); return { year, month, day }; }
function addMonths(parts: Parts, months: number): Parts { const zeroBased = parts.year * 12 + (parts.month - 1) + months; const year = Math.floor(zeroBased / 12); const month = zeroBased - year * 12 + 1; const lastDay = new Date(Date.UTC(year, month, 0)).getUTCDate(); return { year, month, day: Math.min(parts.day, lastDay) }; }
function addDays(parts: Parts, days: number): Parts { const d = new Date(Date.UTC(parts.year, parts.month - 1, parts.day + days)); return { year: d.getUTCFullYear(), month: d.getUTCMonth() + 1, day: d.getUTCDate() }; }
function startOfWeekMonday(parts: Parts): Parts { const day = new Date(Date.UTC(parts.year, parts.month - 1, parts.day)).getUTCDay(); return addDays(parts, day === 0 ? -6 : 1 - day); }
...
function startOfYear(parts: Parts): Parts { return { year: parts.year, month: 1, day: 1 }; }
function toUtcBoundary(parts: Parts, timezone: string): Date {
  ...
  const rendered = new Intl.DateTimeFormat("en-US", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }).formatToParts(naiveUtc);
...
export function explicitDateRange(from: string, to: string, timezone = "UTC"): DateRange {
  const fromParts = parseDate(from); const toParts = parseDate(to); const fromDate = isoDate(fromParts); const toDate = isoDate(toParts);
  if (fromDate > toDate) throw new Error("Analytics date range start must not be after end");
  return { from: fromDate, to: toDate, startAt: toUtcBoundary(fromParts, timezone).toISOString(), endAtExclusive: toUtcBoundary(addDays(toParts, 1), timezone).toISOString(), timezone };
}
