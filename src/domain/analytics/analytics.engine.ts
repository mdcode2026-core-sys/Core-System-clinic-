...
async function getKpiDataSafe(kpiId: string, supabase: AnalyticsSupabaseClient, tenantId: string, datePreset: DatePreset, timezone: string, customFrom?: string, customTo?: string): Promise<KpiResult> {
  ...
  if (!kpiDef) throw new Error(`Unknown KPI: ${kpiId}`);
  try {
    return await getKpiData(kpiId, supabase, tenantId, datePreset, timezone, customFrom, customTo);
  } catch (err) {
    console.error(`[AnalyticsEngine] KPI ${kpiDef.id} failed:`, err);
    return { id: kpiDef.id, nameAr: kpiDef.nameAr, value: "—", raw: 0, timestamp: new Date().toISOString(), coverage: "unknown" };
  }
}
