...
function escapeIlike(value: string) { return value.replace(/\\/g, "\\\\").replace(/%/g, "\\%").replace(/_/g, "\\_"); }

function addUnique(target: GlobalSearchResult[], rows: GlobalSearchResult[]) {
  const seen = new Set(target.map((item) => `${item.type}:${item.id}`));
  for (const row of rows) { const key = `${row.type}:${row.id}`; if (seen.has(key)) continue; target.push(row); seen.add(key); }
}

async function searchText(supabase: Awaited<ReturnType<typeof createClient>>, table: string, tenantId: string, query: string, fields: string[], select: string): Promise<SearchRow[]> {
  const q = escapeIlike(query); const filter = fields.map((field) => `${field}.ilike.%${q}%`).join(",");
...
async function searchByIds(supabase: Awaited<ReturnType<typeof createClient>>, table: string, tenantId: string, field: string, ids: string[], select: string): Promise<SearchRow[]> {
  if (!ids.length) return [];
  const result = await supabase.from(table).select(select).eq("tenant_id", tenantId).in(field, ids).limit(MAX_PER_TYPE);
  return (result.data ?? []) as SearchRow[];
}

async function exactPatient(supabase: Awaited<ReturnType<typeof createClient>>, tenantId: string, id: string): Promise<SearchRow[]> {
...
export async function searchGlobal(query: string, locale: "ar" | "en"): Promise<{ success: true; results: GlobalSearchResult[] } | { success: false; error: string }> {
  ...
  let patientRows: SearchRow[] = has("patients:read") ? await searchText(supabase, "clinic_patients", tenantId, normalized, ["first_name", "last_name", "first_name_ar", "last_name_ar", "phone_primary", "phone_secondary", "email", "file_number"], "id, first_name, last_name, first_name_ar, last_name_ar, phone_primary, email, file_number") : [];
  if (has("patients:read") && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(normalized)) patientRows = [...patientRows, ...(await exactPatient(supabase, tenantId, normalized))];
  addUnique(results, patientRows.map((row) => ({ id: row.id, type: "patient", title: locale === "ar" && row.first_name_ar ? `${row.first_name_ar} ${row.last_name_ar ?? ""}`.trim() : `${row.first_name} ${row.last_name}`.trim(), subtitle: row.file_number ? `#${row.file_number} · ${row.phone_primary ?? row.email ?? ""}` : (row.phone_primary ?? row.email ?? ""), href: `/patients?patientId=${encodeURIComponent(row.id)}` })));
  const patientIds = [...new Set(patientRows.map((row) => row.id as string))];

  const staffRows: SearchRow[] = has("users:read") ? await searchText(supabase, "clinic_users", tenantId, normalized, ["full_name", "full_name_ar", "email", "employee_code", "phone", "specialization", "role"], "id, full_name, full_name_ar, email, employee_code, phone, role") : [];
  ...
  if (has("invoices:read")) {
    ...
    jobs.push(searchByIds(supabase, "clinic_invoices", tenantId, "patient_id", patientIds, "id, invoice_number, invoice_status, total_subunits, amount_due_subunits, patient_id, invoice_date"));
    ...
    jobs.push(searchByIds(supabase, "financial_plans", tenantId, "patient_id", patientIds, "id, patient_id, treatment_plan_id, status, total_amount_subunits, patient_responsibility_subunits, currency, created_at"));
  ...
  if (has("insurance:read")) {
    ...
    jobs.push(searchByIds(supabase, "patient_insurance_profiles", tenantId, "patient_id", patientIds, "id, patient_id, payer_name, policy_number, member_number, status, reconciliation_status"));
  ...
  if (has("agenda:read")) {
    ...
    jobs.push(searchByIds(supabase, "master_agenda_events", tenantId, "patient_id", patientIds, "id, patient_id, doctor_id, scheduled_start, status, event_type, visit_type"));
    jobs.push(searchByIds(supabase, "master_agenda_events", tenantId, "doctor_id", doctorIds, "id, patient_id, doctor_id, scheduled_start, status, event_type, visit_type"));
  }
  if (has("treatment_plans:read")) {
    ...
    jobs.push(searchByIds(supabase, "clinic_treatment_plans", tenantId, "patient_id", patientIds, "id, patient_id, title, status, start_date, target_end_date"));
  }
  ...
  if (has("medical_files:read")) {
    ...
    jobs.push(searchByIds(supabase, "medical_files", tenantId, "patient_id", patientIds, "id, patient_id, original_filename, file_kind, mime_type, created_at"));
  }
