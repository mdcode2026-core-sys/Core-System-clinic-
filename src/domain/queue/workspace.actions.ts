...
export async function registerPatientArrival(data: {
  sessionId?: string;
  patient_id: string;
  doctor_id?: string;
  room_id?: string;
...
export async function transitionToPendingReception(sessionId: string): Promise<EnrichedSession> {
  ...
  const { data: current, error: readError } = await supabase
    .from("clinic_visit_sessions")
    .select("session_status, doctor_id, lock_holder_id")
    .eq("id", sessionId)
    .eq("tenant_id", tenantId)
    .single();
  if (readError || !current) throw new Error("Session not found");
  if (current.doctor_id !== user.id && current.lock_holder_id !== user.id && !permissions.includes("workspace:administration")) {
    throw new Error("This clinical session is not assigned to the current user");
  }
...
export async function markNoShowFromReception(sessionId: string): Promise<EnrichedSession> {
  return transitionSession(sessionId, "no_show", "operation");
}

export async function cancelFromReception(sessionId: string): Promise<EnrichedSession> {
  return transitionSession(sessionId, "cancelled", "operation");
}

export async function moveFromOperation(sessionId: string, target: SessionStatus): Promise<EnrichedSession> {
  if (!["waiting", "in_consultation", "pending_close", "completed", "cancelled", "no_show"].includes(target)) {
...
async function transitionSession(sessionId: string, target: SessionStatus, workspace: "operation" | "clinical") {
  ...
  if (workspace === "clinical" && current.doctor_id !== user.id && current.lock_holder_id !== user.id && !permissions.includes("workspace:administration")) {
    throw new Error("This clinical session is not assigned to the current user");
  }
