...
async function loadPlan(supabase: any, tenantId: string, planId: string): Promise<TreatmentPlanRecord | null> {
  const { data, error } = await supabase.from("clinic_treatment_plans").select(`id,patient_id,source_visit_id,title,diagnosis_summary,goals,status,start_date,target_end_date,completed_at,created_at,updated_at,clinic_patients!clinic_treatment_plans_patient_id_fkey(first_name,last_name),clinic_treatment_plan_items!clinic_treatment_plan_items_treatment_plan_id_fkey(id,treatment_plan_id,procedure_id,title,description,sequence_no,planned_date,quantity,status,completed_at,notes,clinic_procedures(procedure_name)),clinic_treatment_plan_visits!clinic_treatment_plan_visits_treatment_plan_id_fkey(id,treatment_plan_item_id,visit_id,linked_at)`).eq("id", planId).eq("tenant_id", tenantId).single();
