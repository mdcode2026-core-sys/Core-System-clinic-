...
export async function createInvoiceFromSession(
  ...
  // Only receptionist, admin, or super_admin can create invoices
...
export async function issueInvoice(
  ...
  // Verify invoice belongs to tenant and is in draft
...
export async function applyDiscount(
  ...
  // Only admin or super_admin can approve discounts
  if (!["admin", "super_admin"].includes(role)) {
    return { success: false, error: "Only admin can approve discounts" };
  }

  // Verify invoice belongs to tenant and can be edited
  const { data: invoice } = await supabase
    .from("clinic_invoices")
  ...
  // Check if invoice can be edited (15-minute window, no payments)
