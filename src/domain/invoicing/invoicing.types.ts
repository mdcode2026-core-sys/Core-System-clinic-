...
export interface Invoice {
  id: string; tenant_id: string; session_id: string | null; patient_id: string; invoice_number: string | null; invoice_date: string; invoice_status: InvoiceStatus;
  payment_terms: PaymentTerms | null; notes: string | null; issued_at: string | null; subtotal_subunits: number; tax_subunits: number; discount_subunits: number; total_subunits: number;
  amount_paid_subunits: number; amount_due_subunits: number | null; payment_method: string | null; collected_by: string | null; discount_approved_by: string | null; discount_reason: string | null; created_at: string; updated_at: string;
}
export interface InvoiceItem {
  id: string; tenant_id: string; invoice_id: string; procedure_id: string | null; description: string | null; description_ar: string | null; quantity: number; unit_price_subunits: number;
  discount_subunits: number; tax_subunits: number; tax_rate_percent: number | null; line_total_subunits: number | null; sort_order: number; created_at: string; updated_at: string;
}
export interface InvoicePayment {
  id: string; tenant_id: string; invoice_id: string; financial_plan_id: string | null; installment_id: string | null; amount_subunits: number; payment_method: PaymentMethod;
  payment_reference: string | null; transaction_id: string | null; notes: string | null; collected_by: string | null; payment_date: string; created_at: string; updated_at: string;
}
...
export interface CreateManualInvoiceInput { patient_id: string; session_id?: string | null; invoice_date?: string; payment_terms?: PaymentTerms; notes?: string | null; items: { procedure_id?: string | null; description: string; quantity: number; unit_price_subunits: number; discount_amount_subunits?: number; discount_percent?: number | null; discount_reason?: string | null; tax_rate_percent?: number | null }[]; }
