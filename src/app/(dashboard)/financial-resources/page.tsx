...
export default async function FinancialResourcesPage({ searchParams }: { searchParams?: Promise<{ section?: string }> }) {
  ...
  const aliases:Record<string,string>={home:"overview",invoices:"receivables",payments:"receivables",plans:"plans",insurance:"insurance",inventory:"inventory",purchasing:"purchasing",expenses:"expenses"};
  const initialTab=aliases[requestedSection??""];
  const c=await cookies();
  const raw=c.get("core-system-locale")?.value;
  ...
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8" dir={locale==="ar"?"rtl":"ltr"}>
    ...
    {permissions.includes("reports:read")&&<FinancialResourceReport locale={locale} currency={tenant.data?.currency??"JOD"} summary={summary}/>} 
  </div>;
}
