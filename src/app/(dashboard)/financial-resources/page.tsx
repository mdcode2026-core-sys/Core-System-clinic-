...
import { FinancialResourcesCenter, type FinancialPlanSummary, type InsuranceSummary, type InventoryItemSummary, type PatientSummary, type PurchaseOrderSummary, type SupplierSummary, type InvoiceSummary } from "@/features/financial-resources/financial-resources-center";
...
export default async function FinancialResourcesPage({ searchParams }: { searchParams?: Promise<{ section?: string }> }) {
  ...
  const purchaseOrders: PurchaseOrderSummary[] = purchaseResult.success ? purchaseResult.data.map((o) => ({ id: o.id, order_number: o.order_number, order_date: o.order_date, status: o.status, total_subunits: o.total_subunits, supplier: o.supplier, items: (o.items ?? []).map((i) => ({ id: i.id, inventory_item_id: i.inventory_item_id, quantity_ordered: i.quantity_ordered, quantity_received: i.quantity_received, unit_cost_subunits: i.unit_cost_subunits })) })) : [];
