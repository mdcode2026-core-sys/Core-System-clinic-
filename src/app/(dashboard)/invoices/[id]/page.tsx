...
import { InvoiceRefundPanel } from "@/features/invoicing/invoice-refund-panel";
...
export default async function InvoiceDetailPage({ params }: Props) {
  ...
  const refundable = result.data.amount_paid_subunits;
  return <div className="space-y-4 p-6" dir={direction}><InvoiceDetail invoice={result.data} currency={tenant?.currency ?? "USD"} /><InvoiceRefundPanel invoiceId={result.data.id} maxRefundSubunits={refundable} /></div>;
}
