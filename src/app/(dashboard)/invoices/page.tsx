...
export default async function InvoicesPage() {
  ...
  return <div className="space-y-6 p-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    <div><h1 className="text-2xl font-bold">{invoice.title}</h1><p className="mt-1 text-sm text-muted-foreground">Billing and payment collection.</p></div>
    <InvoiceList initialData={result.success ? result.data : []} initialError={result.success ? null : result.error} permissions={permissions} currency={tenant?.currency ?? "USD"} />
  </div>;
}
