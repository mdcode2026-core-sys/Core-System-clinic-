...
export default async function InvoicesPage({ searchParams }: { searchParams?: Promise<{ patientId?: string }> }) {
  ...
  const locale: Locale = localeCookie === "ar" || localeCookie === "en" ? localeCookie : tenantLanguage === "ar" || tenantLanguage === "en" ? tenantLanguage : "en";
  ...
  const [{ data: tenant }, result] = await Promise.all([
    ...
    listInvoices(patientId ? { patient_id: patientId } : {}),
  ]);
  const currency = tenant?.currency ?? "USD";
  return (
    <div className="space-y-6 p-6" dir={locale === "ar" ? "rtl" : "ltr"}>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-2xl font-bold">{invoice.title}</h1><p className="mt-1 text-sm text-muted-foreground">{financial.description}</p>{patientId && <p className="mt-1 text-xs text-muted-foreground">Patient context: {patientId}</p>}</div>
      ...
      <InvoiceList initialData={result.success ? result.data : []} initialError={result.success ? null : result.error} permissions={permissions} currency={currency} />
    </div>
  );
}
