...
import { HomeNavigationLink } from "@/features/home/HomeNavigationLink";
...
export default async function HomePage() {
  ...
  return (
    <div className="cs-page-canvas mx-auto w-full max-w-[1600px] space-y-6" dir={ar ? "rtl" : "ltr"}>
      ...
      {metrics.length > 0 ? <section aria-labelledby="home-today-title"><div className="mb-2 flex items-baseline justify-between gap-3"><h2 id="home-today-title" className="text-lg font-semibold tracking-tight text-[var(--cs-ink-950)]">{ar ? "اليوم" : "Today"}</h2><span className="text-xs text-[var(--cs-slate-500)]">{ar ? "نظرة يومية" : "Daily view"}</span></div><div className="grid grid-cols-2 overflow-hidden rounded-xl border border-[var(--cs-slate-200)] bg-white md:grid-cols-4" data-testid="home-today-actions">{metrics.map((metric, index) => { const Icon = metric.icon; return <HomeNavigationLink key={metric.label} href={metric.href} data-testid={metric.testId} aria-label={`${metric.label}: ${metric.value}`} className={`cs-interactive group flex min-h-[68px] items-center gap-3 px-3 py-3 text-start transition-colors hover:bg-[var(--cs-slate-50)] focus-visible:z-10 md:min-h-[76px] md:px-4 ${index > 0 ? "border-s-[1px] border-[var(--cs-slate-200)]" : ""} ${index === 2 ? "max-md:border-s-0 max-md:border-t" : ""} ${index === 3 ? "max-md:border-t" : ""}`}><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--cs-azure-100)] text-[var(--cs-azure-700)] group-hover:bg-[var(--cs-azure-100)]"><Icon className="h-[18px] w-[18px]" aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-[var(--cs-slate-600)]">{metric.label}</span><span className="mt-0.5 block text-xl font-bold tabular-nums text-[var(--cs-ink-950)]">{metric.value}</span></span></HomeNavigationLink>; })}</div></section> : null}
