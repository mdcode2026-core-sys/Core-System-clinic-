...
const tabs=[
 {id:"team-access",icon:ShieldCheck,permission:"settings:read",key:"teamAccess"},
 ...
 {id:"user-settings",icon:UserRound,permission:null,key:"user"},
] as const;
...
export default function SettingsPage(){
 ...
 const cards=[
  {id:"users",icon:Users,title:team.users,desc:ar?"إنشاء المستخدمين وتفعيلهم وتعيين الأدوار.":"Create, activate, and assign roles to users."},
  {id:"roles",icon:ShieldCheck,title:team.roles,desc:ar?"إنشاء الأدوار وتوزيع صلاحيات Permission Catalog.":"Create roles and assign catalogued permissions."},
  {id:"overrides",icon:UserCog,title:team.overrides,desc:ar?"منح أو سحب صلاحيات لمستخدم محدد.":"Grant or explicitly revoke permissions for a user."},
  {id:"templates",icon:FileText,title:team.templates,desc:ar?"نماذج استرشادية قابلة للتعديل وليست قواعد إلزامية.":"Advisory starting templates, not mandatory policy."},
  {id:"user-settings",icon:UserRound,title:team.settings,desc:ar?"إعداداتك الشخصية منفصلة عن الصلاحيات.":"Personal preferences are separate from authorization."},
 ];
 return <div className="space-y-6"><h1 className="text-2xl font-bold">{a.settings.title}</h1><div className="flex flex-wrap gap-2 border-b border-border pb-2">{visible.map(tab=>{const Icon=tab.icon;const active=activeTab===tab.id;const label=tab.id==="team-access"?team.title:tab.id==="user-settings"?team.settings:((a.settings as any)[tab.key]||tab.key);return <button key={tab.id} onClick={()=>setActiveTab(tab.id)} aria-current={active?"page":undefined} className={`inline-flex items-center gap-2 rounded-t-md px-4 py-2 text-sm font-medium ${active?"border-b-2 border-primary text-primary":"text-muted-foreground hover:text-foreground"}`}><Icon className="h-4 w-4"/>{label}</button>})}</div>
 {activeTab==="team-access"&&<div className="space-y-5" dir={ar?"rtl":"ltr"}><div><h2 className="text-xl font-semibold">{team.title}</h2><p className="text-sm text-muted-foreground">{team.description}</p></div><div className="grid gap-4 md:grid-cols-2">{cards.filter(c=>visible.some(t=>t.id===c.id)).map(c=>{const Icon=c.icon;return <Card key={c.id}><CardContent className="flex items-start justify-between gap-4 p-5"><div className="flex gap-3"><div className="rounded-lg bg-primary/10 p-2 h-fit"><Icon className="h-5 w-5 text-primary"/></div><div><h3 className="font-semibold">{c.title}</h3><p className="mt-1 text-sm text-muted-foreground">{c.desc}</p></div></div><Button variant="outline" size="sm" onClick={()=>setActiveTab(c.id)}>{team.open}</Button></CardContent></Card>})}</div><Card className="bg-muted/50"><CardContent className="p-5"><p className="text-sm text-muted-foreground">{ar?"Role ≠ Permission · Workspace ≠ Security Boundary · الصلاحية الفعلية تخضع للصلاحيات وEntitlement الخاص بالعيادة.":"Role ≠ Permission · Workspace ≠ Security Boundary · Effective access is governed by permissions and tenant entitlement."}</p></CardContent></Card></div>}
