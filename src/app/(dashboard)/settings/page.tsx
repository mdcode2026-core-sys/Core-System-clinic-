...
import { LayoutDashboard, Building2, Users, ShieldCheck, FileText, UserCog, Bell, CreditCard, ClipboardList, Settings, Stethoscope, DoorOpen, UserRound, Search, ChevronRight, ChevronLeft, SlidersHorizontal } from "lucide-react";
...
export default function SettingsPage() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center py-12"><div className="h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent" /><span className="ms-3 text-muted-foreground">{a.users.loading}</span></div>;
  }
  ...
  const labels: Record<string, string> = ar ? {
    overview: "نظرة عامة", clinic: "ملف العيادة", rooms: "الغرف والموارد", teamAccess: "إدارة الفريق والصلاحيات", users: "المستخدمون", roles: "الأدوار والصلاحيات", templates: "قوالب الأدوار", overrides: "الوصول والاستثناءات", user: "إعداداتي", notifications: "الإشعارات", preferences: "تفضيلات النظام", subscription: "الاشتراك", audit: "التدقيق والنشاط", procedures: "الخدمات والإجراءات",
  } : {
    overview: "Overview", clinic: "Clinic Profile", rooms: "Rooms & Resources", teamAccess: "Team & Access", users: "Users", roles: "Roles & Permissions", templates: "Role Templates", overrides: "Access & Overrides", user: "My Settings", notifications: "Notifications", preferences: "System Preferences", subscription: "Subscription", audit: "Audit & Activity", procedures: "Services & Procedures",
  };
  ...
  return <div className="space-y-5" dir={ar ? "rtl" : "ltr"}>
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-sm font-medium text-muted-foreground">{ar ? "الإدارة" : "Administration"}</p><h1 className="mt-1 text-2xl font-bold tracking-tight">{a.settings.title}</h1><p className="mt-1 max-w-2xl text-sm text-muted-foreground">{ar ? "إدارة إعدادات العيادة والفريق والنظام من مساحة واحدة واضحة." : "Manage clinic, team, and system settings from one clear administration space."}</p></div>
      <div className="relative w-full lg:w-80"><Search className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={ar ? "ابحث في الإعدادات..." : "Search settings..."} className="ps-9" aria-label={ar ? "البحث في الإعدادات" : "Search settings"} /></div>
    </div>
    <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-5">
        ...
        {activeTab === "team-access" && <div className="space-y-5"><div><h2 className="text-xl font-semibold">{labels.teamAccess}</h2><p className="text-sm text-muted-foreground">{ar ? "إدارة المستخدمين والأدوار والصلاحيات الفعلية والاستثناءات وإعدادات المستخدم من مكان واحد." : "Manage users, roles, effective access, overrides, and user settings from one place."}</p></div><div className="grid gap-4 md:grid-cols-2">{teamCards.filter((card) => visibleIds.has(card.id)).map((card) => { const Icon = card.icon; return <Card key={card.id} className="transition-shadow hover:shadow-sm"><CardContent className="flex h-full flex-col p-5"><div className="flex items-start gap-3"><div className="rounded-lg bg-primary/10 p-2 text-primary"><Icon className="h-5 w-5" /></div><div><h3 className="font-semibold">{card.title}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{card.description}</p></div></div><button onClick={() => navigate(card.id)} className="mt-5 inline-flex items-center gap-1 self-start text-sm font-medium text-primary hover:underline">{ar ? "فتح" : "Open"}{ar ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}</button></CardContent></Card>; })}</div><Card className="bg-muted/50"><CardContent className="p-5"><p className="text-sm text-muted-foreground">{ar ? "Role ≠ Permission · Workspace ≠ Security Boundary · الصلاحية الفعلية تخضع للصلاحيات وEntitlement الخاص بالعيادة." : "Role ≠ Permission · Workspace ≠ Security Boundary · Effective access is governed by permissions and tenant entitlement."}</p></CardContent></Card></div>}
