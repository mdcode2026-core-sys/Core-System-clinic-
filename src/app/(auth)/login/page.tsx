  ...
  return (
    <Card className="cs-surface-raised mx-auto w-full max-w-md rounded-2xl border-[var(--cs-slate-200)] bg-white" dir={locale === "ar" ? "rtl" : "ltr"}>
      <CardHeader className="space-y-5 p-6 pb-4 text-center sm:p-8 sm:pb-5">
        ...
        <div><CardTitle className="text-2xl font-bold tracking-tight text-[var(--cs-ink-950)]">{t.login}</CardTitle><p className="mt-2 text-sm leading-6 text-[var(--cs-slate-500)]">{locale === "ar" ? "دخول آمن إلى نظام العيادة" : "Secure access to your clinic system"}</p></div>
      </CardHeader>
