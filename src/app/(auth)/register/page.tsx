...
function RegisterForm() {
  ...
  async function handleSubmit(formData: FormData) { setLoading(true); setError(null); const result = await signUp(formData); if (result?.errorCode) { setError(result.errorCode === "AUTH_CLINIC_CREATION_FAILED" ? t.errors.clinicCreationFailed : result.errorCode === "AUTH_SESSION_SETUP_FAILED" ? t.errors.sessionSetupFailed : result.errorCode === "AUTH_SIGN_UP_FAILED" ? t.errors.signUpFailed : t.errors.signUpFailed); setLoading(false); return; } router.push("/check-email"); router.refresh(); }
...
export default function RegisterPage() { const { auth: t } = useI18n(); return <Suspense fallback={<div className="flex min-h-screen items-center justify-center"><div className="text-white">{t.pageLoading}</div></Suspense> }><RegisterForm /></Suspense>; }
