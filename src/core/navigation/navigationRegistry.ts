...
const financialResourcesChildren: NavItem[] = [
  ...
  { href: "/financial-resources?section=purchasing", label: { ar: "المشتريات", en: "Purchasing" }, labelKey: null, icon: ShoppingCart, requiredPermission: "purchasing:read", capabilityKey: "financial_resources.purchasing", surface: "core" },
...
export function getRequiredPermission(pathname: string): Permission | null | undefined {
  ...
  const exact = navigationRegistry.flatMap((n) => n.children ?? [n]).find((n) => n.href.split("?")[0] === normalized);
