...
export function GlobalSearch() {
  ...
  const field = (mobile = false) => (
    <div className={cn("flex min-w-0 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 shadow-sm transition-colors", open && "border-blue-300 bg-white shadow-md", "focus-within:border-blue-300 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-100", mobile ? "w-full" : "w-full")}>
  ...
  return (
    <div ref={wrapperRef} className="relative h-10 w-full min-w-0">
      ...
      {open ? <div className={cn("z-[80] max-h-[min(70vh,32rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl", mobileOpen ? "fixed inset-x-2 top-[7rem]" : "absolute inset-x-0 top-12")}>
        {!hasSearchQuery ? <p className="px-3 py-4 text-sm text-slate-500">{messages.minChars}</p> : isPending ? <p className="flex items-center gap-2 px-3 py-4 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" />{messages.loading}</p> : results.length === 0 ? <p className="px-3 py-4 text-sm text-slate-500">{messages.noResults}</p> : <div className="space-y-1">{results.map((result) => <Link key={`${result.type}-${result.id}`} href={result.href} onClick={() => { setOpen(false); setMobileOpen(false); }} className="flex min-w-0 items-center gap-2 rounded-xl px-3 py-2.5 transition-colors hover:bg-slate-50 sm:gap-3"><span className="max-w-[5rem] shrink-0 truncate text-xs font-medium text-slate-500 sm:min-w-24">{typeLabel(result.type)}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-slate-900">{result.title}</span>{result.subtitle && <span className="block truncate text-xs text-slate-500">{result.subtitle}</span>}</span><ArrowRight className="h-4 w-4 shrink-0 text-slate-400 rtl:rotate-180" aria-hidden="true" /></Link>)}</div>}
      </div> : null}
    </div>
  );
}
