import type { Locale } from "./messages";

export const workspaceMessages = {
  ar: {
    workingSurface: "مساحة العمل",
    layerQuickActions: "إجراءات سريعة",
    layerStatusAnalytics: "الحالة والتحليلات",
    noWidgets: "لا توجد أدوات متاحة",
    contactAdmin: "اتصل بالمسؤول لتمكين الوحدات",
    widgetError: "حدث خطأ غير متوقع",
    retry: "إعادة المحاولة",
    collapsed: "مطوي",
    expand: "توسيع",
    collapse: "طي",
    unpin: "إلغاء التثبيت",
    pin: "تثبيت",
    show: "إظهار",
    hide: "إخفاء",
    customize: "تخصيص مساحة العمل",
    closeCustomizer: "إغلاق التخصيص",
    customizeDescription: "أضف الأدوات المسموح بها، أعد ترتيبها، أو أعد مساحة العمل إلى الإعداد الافتراضي.",
    addWidgets: "إضافة أدوات",
    resetWorkspace: "استعادة الافتراضي",
    dragToReorder: "اسحب لإعادة الترتيب",
    moveUp: "نقل لأعلى",
    moveDown: "نقل لأسفل",
    widgetLibraryTitle: "مكتبة أدوات مساحة العمل",
    widgetLibraryDescription: "اختر الأدوات التي تريد إظهارها في مساحة العمل الحالية.",
    noAvailableWidgets: "لا توجد أدوات مسموحة متاحة للإضافة.",
    add: "إضافة",
    added: "مضافة",
    close: "إغلاق",
    followupWidget: { title: "المتابعة", patients: "متابعات المرضى", description: "اعرف ما يحتاج إلى متابعة اليوم وافتح قائمة العمل.", open: "فتح المتابعة" },
    quickRegistration: { clinicNotSelected: "لم يتم تحديد العيادة", required: "الاسم ورقم الهاتف مطلوبان", success: "تم تسجيل المريض بنجاح", successShort: "تم التسجيل بنجاح", failure: "فشل تسجيل المريض", fullName: "الاسم الكامل *", fullNamePlaceholder: "اسم المريض", phone: "رقم الهاتف *", phonePlaceholder: "رقم الهاتف", gender: "الجنس", male: "ذكر", female: "أنثى", dateOfBirth: "تاريخ الميلاد", submitting: "جاري التسجيل...", submit: "تسجيل المريض" },
    quickAppointment: { clinicNotSelected: "لم يتم تحديد العيادة أو المستخدم", required: "المريض والطبيب ووقت البدء مطلوبون", success: "تم إنشاء الموعد بنجاح", successShort: "تم إنشاء الموعد بنجاح", failure: "فشل إنشاء الموعد", patient: "المريض (ID) *", patientPlaceholder: "معرف المريض", doctor: "الطبيب *", chooseDoctor: "اختر طبيباً", room: "الغرفة", chooseRoom: "اختر غرفة", procedure: "الإجراء", chooseProcedure: "اختر إجراءً", start: "وقت البدء *", end: "وقت الانتهاء", notes: "ملاحظات", notesPlaceholder: "ملاحظات إضافية...", saving: "جاري الحفظ...", submit: "إنشاء الموعد" },
    billing: { collectedRevenue: "الإيرادات المحصّلة", paidInvoices: "فواتير مدفوعة", outstanding: "مستحق", period: "الفترة", thisMonth: "هذا الشهر", loadFailed: "فشل تحميل بيانات الفواتير", noData: "لا توجد بيانات فوترة" },
  },
  en: {
    workingSurface: "Working surface",
    layerQuickActions: "Quick Actions",
    layerStatusAnalytics: "Status & Analytics",
    noWidgets: "No widgets are available",
    contactAdmin: "Contact an administrator to enable modules",
    widgetError: "An unexpected error occurred",
    retry: "Retry",
    collapsed: "Collapsed",
    expand: "Expand",
    collapse: "Collapse",
    unpin: "Unpin",
    pin: "Pin",
    show: "Show",
    hide: "Hide",
    customize: "Customize workspace",
    closeCustomizer: "Close customization",
    customizeDescription: "Add authorized widgets, reorder them, or restore the default workspace configuration.",
    addWidgets: "Add widgets",
    resetWorkspace: "Restore default",
    dragToReorder: "Drag to reorder",
    moveUp: "Move up",
    moveDown: "Move down",
    widgetLibraryTitle: "Workspace widget library",
    widgetLibraryDescription: "Choose the widgets you want to show on the current workspace.",
    noAvailableWidgets: "No authorized widgets are available to add.",
    add: "Add",
    added: "Added",
    close: "Close",
    followupWidget: { title: "Follow-up", patients: "Patient Follow-ups", description: "See what needs follow-up today and open the worklist.", open: "Open follow-up" },
    quickRegistration: { clinicNotSelected: "No clinic is selected", required: "Full name and phone number are required", success: "Patient registered successfully", successShort: "Registration successful", failure: "Patient registration failed", fullName: "Full name *", fullNamePlaceholder: "Patient name", phone: "Phone number *", phonePlaceholder: "Phone number", gender: "Gender", male: "Male", female: "Female", dateOfBirth: "Date of birth", submitting: "Registering...", submit: "Register patient" },
    quickAppointment: { clinicNotSelected: "No clinic or user is selected", required: "Patient, doctor and start time are required", success: "Appointment created successfully", successShort: "Appointment created successfully", failure: "Appointment creation failed", patient: "Patient (ID) *", patientPlaceholder: "Patient ID", doctor: "Doctor *", chooseDoctor: "Choose a doctor", room: "Room", chooseRoom: "Choose a room", procedure: "Procedure", chooseProcedure: "Choose procedure", start: "Start time *", end: "End time", notes: "Notes", notesPlaceholder: "Additional notes...", saving: "Saving...", submit: "Create appointment" },
    billing: { collectedRevenue: "Collected revenue", paidInvoices: "Paid invoices", outstanding: "Outstanding", period: "Period", thisMonth: "This month", loadFailed: "Failed to load billing data", noData: "No billing data" },
  },
} as const satisfies Record<Locale, object>;

export function getWorkspaceMessages(locale: Locale) { return workspaceMessages[locale]; }
